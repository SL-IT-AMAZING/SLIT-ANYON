...
interface AnyonMcpToolResultProps {
  node?: any;
  children?: React.ReactNode;
}

export const AnyonMcpToolResult: React.FC<AnyonMcpToolResultProps> = ({
  node,
  children,
}) => {
  ...
  const prettyJson = useMemo(() => {
    ...
    try {
      ...
      console.error("Error parsing JSON for anyon-mcp-tool-result", e);
      return raw;
    }
  }, [expanded, raw]);
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-3 left-2 flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold text-emerald-600 bg-card"
        style={{ zIndex: 1 }}
      >
      ...
      <div className="flex items-start gap-2 pl-24 pr-8 py-1">
        {serverName ? (
          <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-muted text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-border">
            {serverName}
          </span>
        ) : null}
        {toolName ? (
