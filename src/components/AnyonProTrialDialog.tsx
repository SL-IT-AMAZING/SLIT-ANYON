...
interface AnyonProTrialDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export function AnyonProTrialDialog({
  isOpen,
  onClose,
}: AnyonProTrialDialogProps) {
  const handleStartTrial = () => {
    ipc.system.openExternalUrl(
      "https://pay.any-on.dev/checkout?trialCode=1PRO30&utm_source=anyon-app&utm_medium=app&utm_campaign=setup-dialog-v2",
    );
    onClose();
  };
  ...
  const handleLearnMore = () => {
    ipc.system.openExternalUrl(
      "https://any-on.dev/pro?utm_source=anyon-app&utm_medium=app&utm_campaign=setup-dialog-v2",
    );
  };
