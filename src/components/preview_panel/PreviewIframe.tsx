...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Add a little chip that says "Internal error" if source is "anyon-app" */}
      {error.source === "anyon-app" && (
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : error.source === "anyon-app"
                ? "Try restarting the Anyon app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "anyon-text-updated") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "anyon-text-finalized") {
        handleTextUpdated(event.data);
        return;
      }
      ...
      if (event.data?.type === "anyon-component-deselected") {
        ...
        if (componentId) {
          ...
          if (iframeRef.current?.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              {
                type: "disable-anyon-text-editing",
                data: { componentId },
              },
              "*",
            );
      ...
      if (event.data?.type === "anyon-component-coordinates-updated") {
  ...
  const handleActivateComponentSelector = () => {
    ...
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-anyon-component-selector"
            : "deactivate-anyon-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (!data || data.type !== "anyon-component-selected") {
    return null;
  }
