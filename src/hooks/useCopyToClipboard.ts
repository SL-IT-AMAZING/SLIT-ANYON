...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as AnyonMarkdownParser but convert to clean text
      const formattedContent = convertAnyonContentToMarkdown(messageContent);
  ...
  // Convert Anyon content to clean markdown using the same parsing logic as AnyonMarkdownParser
  const convertAnyonContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from AnyonMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from AnyonMarkdownParser)
  ...
  // Reuse the same parsing functions from AnyonMarkdownParser but simplified
