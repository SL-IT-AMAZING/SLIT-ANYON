...
// Standard dynamic-programming Levenshtein implementation over an O(n*m) matrix.
...
// Replacer chain ordered from strict to increasingly fuzzy matching.
export function replace(
  ...
  for (const replacer of [
    ...
    const matches = replacer(content, oldString);
    while (true) {
      const next = matches.next();
      if (next.done) {
        break;
      }
      const search = next.value;
...
export const editTool: NativeTool<{
  ...
  id: "edit",
  description: "Apply a textual edit by replacing one string with another.",
  parameters: z.object({
    filePath: z.string(),
    oldString: z.string(),
    newString: z.string(),
