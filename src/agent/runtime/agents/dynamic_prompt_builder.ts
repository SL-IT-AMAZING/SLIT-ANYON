...
export function categorizeTools(toolNames: string[]): AvailableTool[] {
  return toolNames.map((name) => {
    ...
    if (name.startsWith("lsp_") || name.startsWith("mcp_lsp_")) {
      category = "lsp";
    } else if (name.startsWith("ast_grep") || name.startsWith("mcp_ast_grep")) {
      category = "ast";
    } else if (
      name === "grep" ||
      name === "glob" ||
      ...
      category = "search";
    } else if (name.startsWith("session_") || name.startsWith("mcp_session_")) {
      category = "session";
    } else if (name === "slashcommand" || name === "mcp_slashcommand") {
      category = "command";
    }
...
export function buildKeyTriggersSection(
  ...
  const skillTriggers = skills
    ...
    .map(
      (s) =>
        `- **Skill \`${s.name}\`**: ${extractTriggerFromDescription(s.description)}`,
    );
...
\`\`\`typescript
task(category="...", load_skills=[], prompt="...")  // Empty skills without justification
\`\`\``;
}
...
export function buildUltraworkSection(
  ...
  if (skills.length > 0) {
    lines.push(
      "**Skills** (combine with categories - EVALUATE ALL for relevance):",
    );
    for (const skill of skills) {
