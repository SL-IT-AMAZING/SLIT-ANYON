...
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Find auth implementations in our codebase...")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Find error handling patterns here...")
// Reference Grep (external)
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Find JWT best practices in official docs...")
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Find how production apps handle auth in Express...")
...
**Example:**
\`\`\`
task(session_id="ses_abc123", prompt="The previous search missed X. Also look for Y.")
\`\`\`
...
export function buildFullSisyphusPrompt(
  availableAgents: AvailableAgent[],
  ...
  return buildDynamicSisyphusPrompt(availableAgents, tools, skills, categories);
}
