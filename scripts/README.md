...
1. `anyon-{version}-1.x86_64.rpm` (Linux RPM)
2. `anyon-{version}-full.nupkg` (Windows NuGet package)
3. `anyon-{version}.Setup.exe` (Windows installer)
4. `anyon-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `anyon-darwin-x64-{version}.zip` (macOS Intel)
6. `anyon_{version}_amd64.deb` (Linux DEB)
