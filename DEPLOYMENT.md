...
| `APPLE_TEAM_ID`       | Apple Developer Team ID         | Find in Apple Developer account → Membership details                                               |
...
| Variable                  | Description                       | How to Obtain                                                                            |
| ------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------- |
| `AZURE_CLIENT_ID`         | Azure service principal client ID | Azure Portal → App registrations → Your app → Application (client) ID                    |
| `AZURE_CLIENT_SECRET`     | Azure service principal secret    | Azure Portal → App registrations → Your app → Certificates & secrets → New client secret |
| `AZURE_TENANT_ID`         | Azure tenant ID                   | Azure Portal → Azure Active Directory → Overview → Tenant ID                             |
...
```
✅ VERIFICATION PASSED!
🎉 All 8 expected assets are present in release vX.Y.Z
```
...
| `AZURE_CLIENT_ID`         | Windows signing              | Service principal ID       | Azure Portal → App registrations           |
| `AZURE_CLIENT_SECRET`     | Windows signing              | Service principal secret   | Azure Portal → Certificates & secrets      |
| `AZURE_TENANT_ID`         | Windows signing              | Azure tenant ID            | Azure Portal → Azure AD → Overview         |
...
- Solution: This is expected for new releases. Users can click "More info" → "Run anyway"
...
Each machine is assigned a stable bucket (0–99) derived from an MD5 hash of `app.getPath('userData')`. The same machine always maps to the same bucket across restarts, so a user won't flip between "included" and "excluded" during a rollout.
...
When `rolloutPercentage` is 25, only users in buckets 0–24 are served the update. The config gates a specific version — unrecognized versions pass through ungated.
...
| --- | ------------------- | ---------------------------- |
| 1   | 10                  | ~10% of users (buckets 0–9)  |
| 3   | 25                  | ~25% of users (buckets 0–24) |
| 5   | 50                  | ~50% of users (buckets 0–49) |
| 7   | 100                 | All users                    |
...
1. Check `forge.config.ts` → `rebuildConfig`:
...
- Check expiration date in Azure Portal → Trusted Signing → Certificate profiles
