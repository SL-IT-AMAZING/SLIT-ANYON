...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-anyon.tsx");
  ...
  await po.page.getByText("made-with-anyon.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithAnyon = ;");
  ...
  expect(editedFile).toContain("export const MadeWithAnyon = ;");
});
