...
testSkipIfWindows(
  ...
  async ({ po }) => {
    // Set up WITHOUT Anyon Pro - use test provider instead
    ...
    await expect(
      po.page.getByRole("button", { name: "Upgrade to Anyon Pro" }),
    ).toBeVisible();
    await expect(
    ...
    await po.sendPrompt("[anyon-qa=write] create a simple file");
    await po.waitForChatCompletion();
  },
);
