(() => {
  ...
  function findElementByAnyonId(anyonId, runtimeId) {
    ...
    if (runtimeId) {
      const elementByRuntimeId = document.querySelector(
        `[data-anyon-runtime-id="${runtimeId}"]`,
      );
    ...
    // Fall back to finding by anyon-id (will get first match)
    const escaped = CSS.escape(anyonId);
    return document.querySelector(`[data-anyon-id="${escaped}"]`);
  }
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByAnyonId(elementId, runtimeId);
    if (element) {
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByAnyonId(componentId, runtimeId);
    if (element) {
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-anyon-component-styles":
        handleGetStyles(data);
        break;
      case "modify-anyon-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-anyon-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-anyon-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-anyon-text-content":
