...
Anyon assigns each agent a model that matches its _working style_ — like building a team where each person is in the role that fits their personality.

### Conductor: The Sociable Lead

Conductor is the developer who knows everyone, goes everywhere, and gets things done through communication and coordination. Talks to other agents, understands context across the whole codebase, delegates work intelligently, and codes well too. But deep, purely technical problems? He'll struggle a bit.

**This is why Conductor uses Claude / Kimi / GLM.** These models excel at:

- Following complex, multi-step instructions (Conductor's prompt is ~1,100 lines)
...
Using Conductor with GPT would be like taking your best project manager — the one who coordinates everyone, runs standups, and keeps the whole team aligned — and sticking them in a room alone to debug a race condition. Wrong fit. No GPT prompt exists for Conductor, and for good reason.
...
Agents that support both families (Strategist, Taskmaster) auto-detect your model at runtime and switch prompts via `isGptModel()`. You don't have to think about it.
...
| ------------ | ----------------- | ------------------------------------------------ | ------------------------------------------ |
| **Conductor** | Main orchestrator | Claude Opus → Kimi K2.5 → GLM 5                  | **No GPT prompt.** Claude-family only.     |
| **Analyst**   | Plan gap analyzer | Claude Opus → Kimi K2.5 → GPT-5.2 → Gemini 3 Pro | Claude preferred, GPT acceptable fallback. |
...
| **Taskmaster** | Todo orchestrator | Kimi K2.5 → Claude Sonnet → GPT-5.2              | Kimi is the sweet spot — Claude-like but cheaper.                    |
...
| **Advisor**   | Architecture consultant | GPT-5.2 → Gemini 3 Pro → Claude Opus | Read-only high-IQ consultation.                  |
...
| Model                 | Strengths                                                                              |
| --------------------- | -------------------------------------------------------------------------------------- |
| **Claude Opus 4.6**   | Best overall. Highest compliance with complex prompts. Default for Conductor.          |
...
| **Kimi K2.5**         | Behaves very similarly to Claude. Great all-rounder at lower cost. Default for Taskmaster. |
...
| **GPT-5.2**       | High intelligence, strategic reasoning. Default for Advisor and Critic.     |
...
- Conductor: Opus → Sonnet, Kimi K2.5, GLM 5 (all communicative models)
- Strategist: Opus → GPT-5.2 (auto-switches to GPT prompt)
- Taskmaster: Kimi K2.5 → Sonnet, GPT-5.2 (auto-switches to GPT prompt)

**Dangerous** — personality mismatch:

- Conductor → GPT: **No GPT prompt exists. Will degrade significantly.**
