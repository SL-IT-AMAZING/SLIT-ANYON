...
Complete reference for the `anyon` command-line interface.

## Basic Usage

```bash
# Display help
bunx @anyon-cli/anyon
...
Interactive installation tool for initial Anyon setup. Provides a TUI based on `@clack/prompts`.
...
3. **Configuration File Creation**: Generates `opencode.json` or `anyon.json` files
4. **Plugin Registration**: Automatically registers the anyon plugin in OpenCode settings
...
Diagnoses your environment to ensure Anyon is functioning correctly. Performs 17+ health checks.

### Usage

```bash
bunx @anyon-cli/anyon doctor
```
...
```bash
bunx @anyon-cli/anyon run [prompt]
```
...
bunx @anyon-cli/anyon mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes "read,write"

# Remove stored OAuth tokens
bunx @anyon-cli/anyon mcp oauth logout <server-name>

# Check OAuth token status
bunx @anyon-cli/anyon mcp oauth status [server-name]
```
...
```bash
# Login
bunx @anyon-cli/anyon auth login

# Logout
bunx @anyon-cli/anyon auth logout

# Check current status
bunx @anyon-cli/anyon auth status
```
...
1. **Project Level**: `.opencode/anyon.json`
2. **User Level**: `~/.config/opencode/anyon.json`
...
bunx @anyon-cli/anyon doctor --verbose

# Check specific category only
bunx @anyon-cli/anyon doctor --category authentication
```
...
bunx @anyon-cli/anyon doctor --no-tui --json

# Save results to file
bunx @anyon-cli/anyon doctor --json > doctor-report.json
```
