...
Complete reference for `anyon.jsonc` configuration. This document covers every available option with examples.
...
- [Task System](#task-system)
  ...
  - [Conductor Agent](#conductor-agent)
  - [Conductor Tasks](#conductor-tasks)
- [Features](#features)
  - [Skills](#skills)
  - [Hooks](#hooks)
  - [Commands](#commands)
...
1. `.opencode/anyon.jsonc` / `.opencode/anyon.json`
2. User config (`.jsonc` preferred over `.json`):

| Platform    | Path                             |
| ----------- | -------------------------------- |
| macOS/Linux | `~/.config/opencode/anyon.jsonc` |
| Windows     | `%APPDATA%\opencode\anyon.jsonc` |
...
Run `bunx @anyon-cli/anyon install` for guided setup. Run `opencode models` to list available models.
...
```jsonc
{
  ...
  "agents": {
    ...
    // Strategist inherits conductor model; just add prompt guidance
    "strategist": {
...
Override built-in agent settings. Available agents: `conductor`, `craftsman`, `strategist`, `advisor`, `researcher`, `scout`, `inspector`, `analyst`, `critic`, `taskmaster`.

```json
{
  "agents": {
    "scout": { "model": "anthropic/claude-haiku-4-5", "temperature": 0.5 },
    "inspector": { "disable": true }
  }
}
```

Disable agents entirely: `{ "disabled_agents": ["advisor", "inspector"] }`
...
{
  "agents": {
    "advisor": { "thinking": { "type": "enabled", "budgetTokens": 200000 } }
  }
}
```
...
Domain-specific model delegation used by the `task()` tool. When Conductor delegates work, it picks a category, not a model name.
...
| ------------- | ------------------ | -------------------------------------------------------------------------- |
| **Conductor**  | `claude-opus-4-6`  | anthropic → github-copilot → opencode → kimi-for-coding → zai-coding-plan  |
| **Craftsman**  | `gpt-5.3-codex`    | openai → github-copilot → opencode                                         |
| **advisor**    | `gpt-5.2`          | openai → google → anthropic (via github-copilot/opencode)                  |
| **researcher** | `glm-4.7`          | zai-coding-plan → opencode → anthropic                                     |
| **scout**      | `grok-code-fast-1` | github-copilot → anthropic/opencode → opencode                             |
| **inspector**  | `gemini-3-flash`   | google → openai → zai-coding-plan → kimi-for-coding → opencode → anthropic |
| **Strategist** | `claude-opus-4-6`  | anthropic → kimi-for-coding → opencode → openai → google                   |
| **Analyst**    | `claude-opus-4-6`  | anthropic → kimi-for-coding → opencode → openai → google                   |
| **Critic**     | `gpt-5.2`          | openai → anthropic → google (via github-copilot/opencode)                  |
| **Taskmaster** | `k2p5`             | kimi-for-coding → opencode → anthropic → openai → google                   |
...
Run `bunx @anyon-cli/anyon doctor --verbose` to see effective model resolution for your config.
...
| Option                    | Default | Description                                                  |
| ------------------------- | ------- | ------------------------------------------------------------ |
| `disabled`                | `false` | Disable all Conductor orchestration, restore original build/plan |
...
Conductor agents can also be customized under `agents` using their names: `Conductor`, `OpenCode-Builder`, `Strategist (Planner)`, `Analyst (Plan Consultant)`.

### Conductor Tasks

Enable the Conductor Tasks system for cross-session task tracking.
...
| Option               | Default        | Description                                |
| -------------------- | -------------- | ------------------------------------------ |
| `enabled`            | `false`        | Enable Conductor Tasks system              |
| `storage_path`       | `.anyon/tasks` | Storage path (relative to project root)    |
...
Available hooks: `todo-continuation-enforcer`, `context-window-monitor`, `session-recovery`, `session-notification`, `comment-checker`, `grep-output-truncator`, `tool-output-truncator`, `directory-agents-injector`, `directory-readme-injector`, `empty-task-response-detector`, `think-mode`, `anthropic-context-window-limit-recovery`, `rules-injector`, `background-notification`, `auto-update-checker`, `startup-toast`, `keyword-detector`, `agent-usage-reminder`, `non-interactive-env`, `interactive-bash-session`, `compaction-context-injector`, `thinking-block-validator`, `claude-code-hooks`, `persist-loop`, `preemptive-compaction`, `auto-slash-command`, `worker-notepad`, `no-conductor-gpt`, `start-work`, `runtime-fallback`

**Notes:**

- `directory-agents-injector` — auto-disabled on OpenCode 1.1.37+ (native AGENTS.md support)
- `no-conductor-gpt` — **do not disable**. Conductor is not optimized for GPT; this hook switches to Craftsman automatically.
...
| `disable_omo_env`                        | `false`    | Disable auto-injected `<anyon-env>` block (date/time/locale). Improves cache hit rate. |
| `task_system`                            | `false`    | Enable Conductor task system                                                           |
...
{
  "agents": {
    "scout": { "model": "ollama/qwen3-coder", "stream": false }
  }
}
```
