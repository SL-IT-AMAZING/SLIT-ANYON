...
The principles and philosophy behind Anyon.
...
**Anyon is built on this premise**: Human intervention during agentic work is fundamentally a wrong signal. If the system is designed correctly, the agent should complete the work without requiring you to babysit it.
...
### Approach 1: Newton (Interview Mode)
...
### Approach 2: Turbo (Just Do It Mode)
...
- Escalation (to Socrates, to user) only when truly needed
...
Everything in Anyon is designed to make this loop work:

| Feature             | Purpose                                       |
| ------------------- | --------------------------------------------- |
| Newton              | Extract intent through intelligent interview  |
| Lovelace            | Catch ambiguities before they become bugs     |
| Nietzsche           | Verify plans are complete before execution    |
...
3. Confirm the plan (or just let turbo handle it)
4. Walk away
