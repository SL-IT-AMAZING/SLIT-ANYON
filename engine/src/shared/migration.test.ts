...
describe("migrateAgentNames", () => {
  test("migrates legacy OmO names to lowercase", () => {
    ...
    expect(migrated["conductor"]).toEqual({ temperature: 0.5 });
  ...
  test("preserves current agent names unchanged", () => {
    ...
    expect(migrated["advisor"]).toEqual({ model: "openai/gpt-5.2" });
    expect(migrated["researcher"]).toEqual({ model: "google/gemini-3-flash" });
  ...
  test("handles case-insensitive migration", () => {
    ...
    const agents = {
      EULER: { model: "test" },
      "planner-euler": { prompt: "test" },
      "Orchestrator-Euler": { model: "openai/gpt-5.2" },
    };
    ...
    expect(migrated["conductor"]).toEqual({ model: "test" });
    expect(migrated["strategist"]).toEqual({ prompt: "test" });
    expect(migrated["taskmaster"]).toEqual({ model: "openai/gpt-5.2" });
  });
  ...
  test("migrates orchestrator-euler to taskmaster", () => {
    // given: Config with legacy orchestrator-euler agent name
    const agents = {
      "orchestrator-euler": { model: "anthropic/claude-opus-4-6" },
    };

    // when: Migrate agent names
    const { migrated, changed } = migrateAgentNames(agents);

    // then: orchestrator-euler should be migrated to taskmaster
    expect(changed).toBe(true);
    expect(migrated["taskmaster"]).toEqual({ model: "anthropic/claude-opus-4-6" });
    expect(migrated["orchestrator-euler"]).toBeUndefined();
  });

  test("migrates lowercase turing to taskmaster", () => {
    // given: Config with lowercase turing agent name
    const agents = {
      turing: { model: "anthropic/claude-opus-4-6" },
    };

    // when: Migrate agent names
    const { migrated, changed } = migrateAgentNames(agents);

    // then: lowercase turing should be migrated to taskmaster
    expect(changed).toBe(true);
    expect(migrated["taskmaster"]).toEqual({ model: "anthropic/claude-opus-4-6" });
  });

  test("migrates Euler variants to lowercase", () => {
    // given agents config with "Euler" key
    // when migrateAgentNames called
    // then key becomes "conductor"
    const agents = { Euler: { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["conductor"]).toEqual({ model: "test" });
    expect(migrated["Euler"]).toBeUndefined();
  });

  test("migrates omo key to conductor", () => {
  ...
  test("migrates Turing variants to lowercase", () => {
    // given agents config with "Turing" key
    // when migrateAgentNames called
    // then key becomes "taskmaster"
    const agents = { Turing: { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["taskmaster"]).toEqual({ model: "test" });
    expect(migrated["Turing"]).toBeUndefined();
  });

  test("migrates Newton variants to lowercase", () => {
    // given agents config with "Newton (Planner)" key
    // when migrateAgentNames called
    // then key becomes "strategist"
    const agents = { "Newton (Planner)": { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["strategist"]).toEqual({ model: "test" });
    expect(migrated["Newton (Planner)"]).toBeUndefined();
  });

  test("migrates Lovelace variants to lowercase", () => {
    // given agents config with "Lovelace (Plan Consultant)" key
    // when migrateAgentNames called
    // then key becomes "analyst"
    const agents = { "Lovelace (Plan Consultant)": { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["analyst"]).toEqual({ model: "test" });
    expect(migrated["Lovelace (Plan Consultant)"]).toBeUndefined();
  });

  test("migrates Nietzsche variants to lowercase", () => {
    // given agents config with "Nietzsche (Plan Reviewer)" key
    // when migrateAgentNames called
    // then key becomes "critic"
    const agents = { "Nietzsche (Plan Reviewer)": { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["critic"]).toEqual({ model: "test" });
    expect(migrated["Nietzsche (Plan Reviewer)"]).toBeUndefined();
  });

  test("migrates Euler-Junior to lowercase", () => {
    // given agents config with "Euler-Junior" key
    // when migrateAgentNames called
    // then key becomes "worker"
    const agents = { "Euler-Junior": { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(true);
    expect(migrated["worker"]).toEqual({ model: "test" });
    expect(migrated["Euler-Junior"]).toBeUndefined();
  });

  test("preserves lowercase passthrough", () => {
    // given agents config with "advisor" key
    // when migrateAgentNames called
    // then key remains "advisor" (no change needed)
    const agents = { advisor: { model: "test" } };
    const { migrated, changed } = migrateAgentNames(agents);
    expect(changed).toBe(false);
    expect(migrated["advisor"]).toEqual({ model: "test" });
  });
});
...
describe("migrateHookNames", () => {
  ...
  test("migrates euler-orchestrator to taskmaster", () => {
    // given: Config with legacy euler-orchestrator hook
    const hooks = ["euler-orchestrator", "comment-checker"];

    // when: Migrate hook names
    const { migrated, changed, removed } = migrateHookNames(hooks);

    // then: euler-orchestrator should be migrated to taskmaster
    expect(changed).toBe(true);
    expect(migrated).toContain("taskmaster");
    expect(migrated).toContain("comment-checker");
    expect(migrated).not.toContain("euler-orchestrator");
    expect(removed).toEqual([]);
  });
...
describe("migrateConfigFile", () => {
  ...
  test("migrates omo_agent to conductor_agent", () => {
    ...
    // then: omo_agent should be migrated to conductor_agent
    expect(needsWrite).toBe(true);
    expect(rawConfig.conductor_agent).toEqual({ disabled: false });
    expect(rawConfig.omo_agent).toBeUndefined();
  });

  test("migrates euler_agent to conductor_agent", () => {
    // given: Config with legacy euler_agent key
    const rawConfig: Record<string, unknown> = {
      euler_agent: { disabled: false },
    };

    // when: Migrate config file
    const needsWrite = migrateConfigFile(testConfigPath, rawConfig);

    // then: euler_agent should be migrated to conductor_agent
    expect(needsWrite).toBe(true);
    expect(rawConfig.conductor_agent).toEqual({ disabled: false });
    expect(rawConfig.euler_agent).toBeUndefined();
  });
  ...
  test("does not write if no migration needed", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      conductor_agent: { disabled: false },
      agents: {
  ...
  test("does not migrate gpt-5.2-codex model versions in agents", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        conductor: { model: "openai/gpt-5.2-codex", temperature: 0.1 },
      },
    };
    ...
    expect(agents["conductor"].model).toBe("openai/gpt-5.2-codex");
  });
  ...
  test("does not set needsWrite when no model versions need migration", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        conductor: { model: "openai/gpt-5.2-codex" },
      },
      categories: {
...
describe("migration maps", () => {
  test("AGENT_NAME_MAP contains all expected legacy mappings", () => {
    ...
    expect(AGENT_NAME_MAP["omo"]).toBe("conductor");
    expect(AGENT_NAME_MAP["OmO"]).toBe("conductor");
    expect(AGENT_NAME_MAP["OmO-Plan"]).toBe("strategist");
    expect(AGENT_NAME_MAP["omo-plan"]).toBe("strategist");
    expect(AGENT_NAME_MAP["Planner-Euler"]).toBe("strategist");
    expect(AGENT_NAME_MAP["plan-consultant"]).toBe("analyst");
  });
...
describe("migrateModelVersions", () => {
  test("#given a config with gpt-5.2-codex model #when migrating model versions #then does not overwrite with non-existent gpt-5.3-codex", () => {
    ...
    expect(changed).toBe(false);
    const conductor = migrated["conductor"] as Record<string, unknown>;
    expect(conductor.model).toBe("openai/gpt-5.2-codex");
    expect(conductor.temperature).toBe(0.1);
  });
  ...
  test("replaces anthropic model version", () => {
    ...
    expect(changed).toBe(true);
    const strategist = migrated["strategist"] as Record<string, unknown>;
    expect(strategist.model).toBe("anthropic/claude-opus-4-6");
  });
  ...
  test("handles agent config with no model field", () => {
    ...
    const agents = {
      conductor: { temperature: 0.1, prompt: "custom" },
    };
  ...
  test("handles agent config with non-string model", () => {
    ...
    const agents = {
      conductor: { model: 123, temperature: 0.1 },
    };
  ...
  test("migrates multiple agents in one pass", () => {
    ...
    expect(changed).toBe(true);
    expect((migrated["conductor"] as Record<string, unknown>).model).toBe(
      "openai/gpt-5.2-codex",
    );
    expect((migrated["strategist"] as Record<string, unknown>).model).toBe(
      "anthropic/claude-opus-4-6",
    );
    expect((migrated["advisor"] as Record<string, unknown>).model).toBe(
      "openai/gpt-5.2",
    );
  });
  ...
  test("handles mixed: some applied, some new", () => {
    ...
    // then: Only strategist should be migrated
    expect(changed).toBe(true);
    expect(newMigrations).toEqual([
...
describe("migrateConfigFile _migrations tracking", () => {
  test("records migrations in _migrations field", () => {
    ...
    const configPath = `${tmpDir}/anyon.json`;
    const rawConfig: Record<string, unknown> = {
      agents: {
        conductor: { model: "openai/gpt-5.2-codex" },
      },
  ...
  test("skips re-migration when _migrations contains the key", () => {
    ...
    const conductor = (rawConfig.agents as Record<string, Record<string, unknown>>)
      .conductor;
    expect(conductor.model).toBe("openai/gpt-5.2-codex");
...
describe("migrateConfigFile with backup", () => {
  ...
  test("preserves model setting without auto-conversion to category", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        inspector: { model: "anthropic/claude-haiku-4-5" },
        advisor: { model: "openai/gpt-5.2" },
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>;
    expect(agents["inspector"].model).toBe("anthropic/claude-haiku-4-5");
  ...
  test("preserves category setting when explicitly set", () => {
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>;
    expect(agents["inspector"].category).toBe("quick");
...
describe("migrateModelVersions with applied migrations", () => {
  ...
  test("handles mixed: skip applied, apply new", () => {
    ...
    // then: Skip conductor (already applied), apply advisor
    expect(changed).toBe(true);
    expect(newMigrations).toEqual([
      "model-version:anthropic/claude-opus-4-5->anthropic/claude-opus-4-6",
    ]);
...
describe("migrateConfigFile with _migrations tracking", () => {
  ...
  test("records new migrations in _migrations field", () => {
    ...
    expect(rawConfig._migrations).toBeUndefined();
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).conductor.model,
  ...
  test("preserves existing _migrations and appends new ones", () => {
    ...
    // then: Should skip conductor, migrate advisor, append to _migrations
    expect(needsWrite).toBe(true);
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).conductor.model,
    ).toBe("openai/gpt-5.2-codex");
    expect(
      (rawConfig.agents as Record<string, Record<string, unknown>>).advisor
        .model,
