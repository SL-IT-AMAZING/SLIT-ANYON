...
import { getAnyonCacheDir } from "./data-path";
...
function getCacheFilePath(filename: string): string {
  return join(getAnyonCacheDir(), filename);
}

function ensureCacheDir(): void {
  const cacheDir = getAnyonCacheDir();
...
export function readConnectedProvidersCache(): string[] | null {
  ...
  try {
    ...
    log("[connected-providers-cache] Error reading cache", {
      error: String(err),
    });
    return null;
  }
...
function writeConnectedProvidersCache(connected: string[]): void {
  ...
  try {
    ...
    log("[connected-providers-cache] Cache written", {
      count: connected.length,
    });
  } catch (err) {
    log("[connected-providers-cache] Error writing cache", {
      error: String(err),
    });
  }
}
...
export function readProviderModelsCache(): ProviderModelsCache | null {
  ...
  try {
    ...
    log("[connected-providers-cache] Error reading provider-models cache", {
      error: String(err),
    });
    return null;
  }
...
export function writeProviderModelsCache(data: {
  ...
  try {
    ...
    log("[connected-providers-cache] Error writing provider-models cache", {
      error: String(err),
    });
  }
}
