/**
 * Detects external plugins that may conflict with anyon features.
...
/**
 * Known notification plugins that conflict with anyon's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[anyon] External notification plugin detected: ${pluginName}

Both anyon and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   anyon's session-notification has been auto-disabled.

   To use anyon's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in anyon.json`;
}
