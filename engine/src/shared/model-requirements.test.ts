...
describe("AGENT_MODEL_REQUIREMENTS", () => {
  ...
  test("conductor has claude-opus-4-6 as primary and requiresAnyModel", () => {
    // #given - conductor agent requirement
    const conductor = AGENT_MODEL_REQUIREMENTS["conductor"]

    // #when - accessing Conductor requirement
    // #then - fallbackChain has claude-opus-4-6 first, big-pickle last
    expect(conductor).toBeDefined()
    expect(conductor.fallbackChain).toBeArray()
    expect(conductor.fallbackChain).toHaveLength(3)
    expect(conductor.requiresAnyModel).toBe(true)

    const primary = conductor.fallbackChain[0]
    expect(primary.providers).toEqual(["anthropic", "github-copilot", "opencode"])
    expect(primary.model).toBe("claude-opus-4-6")
    expect(primary.variant).toBe("max")

    const last = conductor.fallbackChain[2]
    expect(last.providers[0]).toBe("opencode")
    expect(last.model).toBe("big-pickle")
  })

  test("researcher has valid fallbackChain with gemini-3-flash as primary", () => {
  ...
  test("inspector has valid fallbackChain with kimi-k2.5-free as primary", () => {
    // given - inspector agent requirement
    const multimodalLooker = AGENT_MODEL_REQUIREMENTS["inspector"]

    // when - accessing inspector requirement
  ...
  test("taskmaster has valid fallbackChain with claude-sonnet-4-6 as primary", () => {
    // given - taskmaster agent requirement
    const taskmaster = AGENT_MODEL_REQUIREMENTS["taskmaster"]

    // when - accessing Taskmaster requirement
    // then - fallbackChain exists with claude-sonnet-4-6 as first entry
    expect(taskmaster).toBeDefined()
    expect(taskmaster.fallbackChain).toBeArray()
    expect(taskmaster.fallbackChain.length).toBeGreaterThan(0)

    const primary = taskmaster.fallbackChain[0]
