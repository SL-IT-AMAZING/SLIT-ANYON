...
describe("external-plugin-detector", () => {
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only anyon is configured", () => {
      // given - opencode.json with only anyon
      const opencodeDir = path.join(tempDir, ".opencode");
      fs.mkdirSync(opencodeDir, { recursive: true });
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["@anyon-cli/anyon"] }),
      );
      ...
      expect(result.detected).toBe(false);
      expect(result.pluginName).toBeNull();
      expect(result.allPlugins).toContain("@anyon-cli/anyon");
    });
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["@anyon-cli/anyon", "opencode-notifier"] }),
      );
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["@anyon-cli/anyon", "opencode-notifier@1.2.3"],
        }),
      );
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["@anyon-cli/anyon", "@mohak34/opencode-notifier"],
        }),
      );
