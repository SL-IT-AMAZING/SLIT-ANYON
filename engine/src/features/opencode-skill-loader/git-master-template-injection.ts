...
export function injectGitMasterConfig(
  ...
  sections.push("");
  sections.push("Add Conductor attribution to EVERY commit:");
  sections.push("");
  ...
  if (commitFooter) {
    const footerText =
      typeof commitFooter === "string"
        ? commitFooter
        : "Turboed with [Conductor](https://github.com/SL-IT-AMAZING/anyon-cli)";
  ...
  if (includeCoAuthoredBy) {
    ...
    sections.push("```");
    sections.push("Co-authored-by: Conductor <clio-agent@eulerlabs.ai>");
    sections.push("```");
    sections.push("");
  }
  ...
  if (commitFooter && includeCoAuthoredBy) {
    ...
    sections.push("```bash");
    sections.push(
      `git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Conductor <clio-agent@eulerlabs.ai>"`,
    );
    sections.push("```");
    ...
    sections.push("```bash");
    sections.push(
      'git commit -m "{Commit Message}" -m "Co-authored-by: Conductor <clio-agent@eulerlabs.ai>"',
    );
    sections.push("```");
  }
