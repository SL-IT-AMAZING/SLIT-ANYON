    ...
    describe("spawn failure recovery", () => {
        ...
        expect(
          logSpy.mock.calls.some(([message]) =>
            String(message).includes(
              "failed to query window state, deferring session",
            ),
          ),
        ).toBe(true);
      ...
      test("#given spawn fails without close action #when onSessionCreated fires #then session is enqueued in deferred queue", async () => {
        ...
        expect(
          logSpy.mock.calls.some(([message]) =>
            String(message).includes(
              "re-queueing deferred session after spawn failure",
            ),
          ),
        ).toBe(true);
          ...
          agentPanes: [
            {
              ...
              left: 100,
              top: 0,
              title: "anyon-subagent-Timeout Task",
              isActive: false,
            },
          ],
        });
