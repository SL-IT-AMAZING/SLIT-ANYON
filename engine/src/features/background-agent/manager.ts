...
export class BackgroundManager {
  ...
  private async startTask(item: QueueItem): Promise<void> {
    ...
    // Include model if caller provided one (e.g., from Conductor category configs)
      ...
      try {
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
          ...
          const normalizedTools = isRecord(info?.tools)
            ? normalizePromptTools(
                info.tools as Record<
                  string,
                  boolean | "allow" | "deny" | "ask"
                >,
              )
            : undefined;
          if (
      ...
      for (const task of this.tasks.values()) {
        ...
        try {
          ...
          if (sessionStatus?.type === "retry") {
            ...
            if (
              this.tryFallbackRetry(task, errorInfo, "polling:session.status")
            ) {
              continue;
            }
          }
