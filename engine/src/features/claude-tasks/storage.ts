...
import type { AnyonConfig } from "../../config/schema";
import { getOpenCodeConfigDir } from "../../shared/opencode-config-dir";

type TaskConfigWithConductorKey = Partial<AnyonConfig> & {
  conductor?: {
    tasks?: {
      storage_path?: string;
      task_list_id?: string;
...
function getTasksConfig(config: Partial<AnyonConfig>) {
  const configWithConductor = config as TaskConfigWithConductorKey;
  return configWithConductor.conductor?.tasks;
}

export function getTaskDir(config: Partial<AnyonConfig> = {}): string {
  const tasksConfig = getTasksConfig(config);
...
export function resolveTaskListId(config: Partial<AnyonConfig> = {}): string {
  const envId = process.env.TURBO_TASK_LIST_ID?.trim();
  ...
  const configId = getTasksConfig(config)?.task_list_id?.trim();
...
export function listTaskFiles(config: Partial<AnyonConfig> = {}): string[] {
