...
describe("getTaskDir", () => {
  const originalTaskListId = process.env.TURBO_TASK_LIST_ID;
  const originalClaudeTaskListId = process.env.CLAUDE_CODE_TASK_LIST_ID;

  beforeEach(() => {
    if (originalTaskListId === undefined) {
      delete process.env.TURBO_TASK_LIST_ID;
    } else {
      process.env.TURBO_TASK_LIST_ID = originalTaskListId;
    }
  ...
  test("returns global config path for default config", () => {
    //#given
    const config: Partial<AnyonConfig> = {};
  ...
  test("respects TURBO_TASK_LIST_ID env var", () => {
    //#given
    process.env.TURBO_TASK_LIST_ID = "custom list/id";
    const configDir = getOpenCodeConfigDir({ binary: "opencode" });
  ...
  test("respects CLAUDE_CODE_TASK_LIST_ID env var when TURBO_TASK_LIST_ID not set", () => {
    //#given
    delete process.env.TURBO_TASK_LIST_ID;
...
describe("resolveTaskListId", () => {
  ...
  test("returns env var when set", () => {
    //#given
    process.env.TURBO_TASK_LIST_ID = "custom-list";
  ...
  test("returns CLAUDE_CODE_TASK_LIST_ID when TURBO_TASK_LIST_ID not set", () => {
    //#given
    delete process.env.TURBO_TASK_LIST_ID;
...
describe("listTaskFiles", () => {
  ...
  test("returns empty array for non-existent directory", () => {
    ...
    const config: Partial<AnyonConfig> = {
      ...
      conductor: { tasks: { storage_path: TEST_DIR, claude_code_compat: false } },
    };
