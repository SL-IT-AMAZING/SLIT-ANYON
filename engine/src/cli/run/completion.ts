...
export async function checkCompletionConditions(
  ...
  try {
    const continuationState = getContinuationState(
      ctx.directory,
      ctx.sessionID,
    );
...
function areContinuationHooksIdle(
  ...
  if (continuationState.hasActiveThesis) {
    logWaiting(ctx, "thesis continuation is active");
    return false;
  }

  if (continuationState.hasActivePersistLoop) {
    logWaiting(ctx, "persist-loop continuation is active");
    return false;
  }
