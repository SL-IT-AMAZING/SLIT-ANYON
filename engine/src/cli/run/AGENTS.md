...
37 files. Powers the `anyon run <message>` command. Connects to OpenCode server, creates/resumes sessions, streams events, and polls for completion.
...
runner.ts
  ...
  3. agent-resolver.ts → Flag → env → config → Conductor
...
| `agent-resolver.ts`         | Resolve agent: `--agent` flag → `OPENCODE_AGENT` env → config → Conductor |
