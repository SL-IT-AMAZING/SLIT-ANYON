...
import { ANTIGRAVITY_PROVIDER_CONFIG, fetchNpmDistTags, generateAnyonConfig, getPluginNameWithVersion } from "./config-manager"
...
describe("getPluginNameWithVersion", () => {
  ...
  test("returns @latest when current version matches latest tag", async () => {
    ...
    expect(result).toBe("@anyon-cli/anyon@latest")
  })
  ...
  test("returns @beta when current version matches beta tag", async () => {
    ...
    expect(result).toBe("@anyon-cli/anyon@beta")
  })
  ...
  test("returns @next when current version matches next tag", async () => {
    ...
    expect(result).toBe("@anyon-cli/anyon@next")
  })
  ...
  test("returns bare package name when npm returns non-ok response for stable version", async () => {
    ...
    expect(result).toBe("@anyon-cli/anyon")
  })
...
describe("fetchNpmDistTags", () => {
  ...
  test("returns dist-tags on success", async () => {
    ...
    const result = await fetchNpmDistTags("anyon")
...
describe("generateAnyonConfig - model fallback system", () => {
  test("uses github-copilot sonnet fallback when only copilot available", () => {
    ...
    const result = generateAnyonConfig(config)

    // #then Conductor uses Copilot (OR logic - copilot is in claude-opus-4-6 providers)
    expect((result.agents as Record<string, { model: string }>).conductor.model).toBe("github-copilot/claude-opus-4.6")
  })
  ...
  test("uses ultimate fallback when no providers configured", () => {
    ...
    // #then Conductor is omitted (requires all fallback providers)
    expect(result.$schema).toBe("https://raw.githubusercontent.com/SL-IT-AMAZING/anyon-cli/dev/assets/anyon.schema.json")
    expect((result.agents as Record<string, { model: string }>).anyon).toBeUndefined()
  })
  ...
  test("uses ZAI model for researcher when Z.ai is available", () => {
    ...
    // #then researcher should use ZAI model
    expect((result.agents as Record<string, { model: string }>).researcher.model).toBe("zai-coding-plan/glm-4.7")
    // #then Conductor uses Claude (OR logic)
    expect((result.agents as Record<string, { model: string }>).conductor.model).toBe("anthropic/claude-opus-4-6")
  })
  ...
  test("uses native OpenAI models when only ChatGPT available", () => {
    ...
    // #then Conductor is omitted (requires all fallback providers)
    expect((result.agents as Record<string, { model: string }>).conductor).toBeUndefined()
    ...
    // #then inspector should use native OpenAI (first fallback entry is gpt-5.3-codex)
    expect((result.agents as Record<string, { model: string }>).inspector.model).toBe("openai/gpt-5.3-codex")
  })
