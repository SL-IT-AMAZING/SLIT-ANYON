...
export async function runTuiInstaller(
  ...
  spinner.start("Adding anyon to OpenCode config");
  ...
  if (config.hasGemini) {
    ...
    spinner.stop(
      `Provider config added to ${color.cyan(providerResult.configPath)}`,
    );
  }

  spinner.start("Writing anyon configuration");
  ...
  p.note(
    `Include ${color.cyan("turbo")} (or ${color.cyan("ulw")}) in your prompt.\n` +
