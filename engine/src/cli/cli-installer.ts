...
export async function runCliInstaller(
  ...
  if (!validation.valid) {
    ...
    console.log();
    printInfo(
      "Usage: bunx @anyon-cli/anyon install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    );
    console.log();
    return 1;
  }
  ...
  const needsProviderSetup =
    config.hasGemini || config.hasOpenAI || config.hasCopilot;
  ...
  printStep(step++, totalSteps, "Writing anyon configuration...");
  const omoResult = writeAnyonConfig(config);
  ...
  if (!config.hasClaude) {
    ...
    console.log(
      color.red(
        color.bold("  Conductor agent is STRONGLY optimized for Claude Opus 4.5."),
      ),
    );
    console.log(
      color.red(
  ...
  printBox(
    `${color.bold("Pro Tip:")} Include ${color.cyan("turbo")} (or ${color.cyan("ulw")}) in your prompt.\n` +
  ...
  console.log(
    `  ${color.dim("gh api --silent --method PUT /user/starred/SL-IT-AMAZING/anyon-cli >/dev/null 2>&1 || true")}`,
  );
  console.log();
