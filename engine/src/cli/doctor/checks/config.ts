...
import { AnyonConfigSchema } from "../../../config";
import {
  detectConfigFile,
...
function validateConfig(): ConfigValidationResult {
  ...
  try {
    ...
    const rawConfig = parseJsonc<AnyonConfig>(content);
    const schemaResult = AnyonConfigSchema.safeParse(rawConfig);
...
function collectModelResolutionIssues(config: AnyonConfig): DoctorIssue[] {
