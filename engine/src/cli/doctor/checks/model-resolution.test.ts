...
describe("model-resolution check", () => {
  describe("getModelResolutionInfo", () => {
    ...
    it("returns agent requirements with provider chains", async () => {
      ...
      const conductor = info.agents.find((a) => a.name === "conductor");
      expect(conductor).toBeDefined();
      expect(conductor!.requirement.fallbackChain[0]?.model).toBe(
        "claude-opus-4-6",
      );
      expect(conductor!.requirement.fallbackChain[0]?.providers).toContain(
        "anthropic",
      );
    });
    ...
    it("returns category requirements with provider chains", async () => {
      ...
      expect(visual!.requirement.fallbackChain[0]?.model).toBe(
        "gemini-3.1-pro",
      );
  ...
  describe("getModelResolutionInfoWithOverrides", () => {
    // given: User has overrides in anyon.json
    ...
    it("shows user override for agent when configured", async () => {
      ...
      // given: User has override for advisor agent
      const mockConfig = {
        agents: {
      ...
      // then: Advisor should show the override
      const advisor = info.agents.find((a) => a.name === "advisor");
      expect(advisor).toBeDefined();
      expect(advisor!.userOverride).toBe("anthropic/claude-opus-4-6");
      expect(advisor!.effectiveResolution).toBe(
        "User override: anthropic/claude-opus-4-6",
      );
    });
    ...
    it("shows provider fallback when no override exists", async () => {
      ...
      const conductor = info.agents.find((a) => a.name === "conductor");
      expect(conductor).toBeDefined();
      expect(conductor!.userOverride).toBeUndefined();
      expect(conductor!.effectiveResolution).toContain("Provider fallback:");
      expect(conductor!.effectiveResolution).toContain("anthropic");
    });
    ...
    it("captures user variant for agent when configured", async () => {
      ...
      //#given User has model with variant override for advisor agent
      const mockConfig = {
        agents: {
          advisor: { model: "openai/gpt-5.2", variant: "xhigh" },
        },
      };
      ...
      //#then Advisor should have userVariant set
      const advisor = info.agents.find((a) => a.name === "advisor");
      expect(advisor).toBeDefined();
      expect(advisor!.userOverride).toBe("openai/gpt-5.2");
      expect(advisor!.userVariant).toBe("xhigh");
    });
