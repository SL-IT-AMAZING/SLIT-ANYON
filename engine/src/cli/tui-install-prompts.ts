...
export async function promptInstallConfig(
  ...
  const claude = await selectOrCancel<ClaudeSubscription>({
    ...
    options: [
      ...
      },
      {
        ...
        hint: "Full power with Claude Sonnet 4.6 for Researcher",
      },
    ],
  ...
  const openai = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Advisor will use fallback models" },
      {
        value: "yes",
        label: "Yes",
        hint: "GPT-5.2 for Advisor (high-IQ debugging)",
      },
    ],
  ...
  const zaiCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Fallback for Researcher and Multimodal Looker",
      },
    ],
  ...
  const kimiForCoding = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Kimi K2.5 for Conductor/Strategist fallback",
      },
    ],
