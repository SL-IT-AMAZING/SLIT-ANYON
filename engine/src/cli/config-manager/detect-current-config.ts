...
import { getAnyonConfigPath } from "./config-context";
import { detectConfigFormat } from "./opencode-config-format";
import { parseOpenCodeConfigFileWithError } from "./parse-opencode-config-file";

function detectProvidersFromAnyonConfig(): {
  hasOpenAI: boolean;
  ...
  try {
    ...
    const anyonConfig = parseJsonc<Record<string, unknown>>(content);
    if (!anyonConfig || typeof anyonConfig !== "object") {
      return {
        hasOpenAI: true,
        hasOpencodeZen: true,
        hasZaiCodingPlan: false,
    ...
    const configStr = JSON.stringify(anyonConfig);
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  result.isInstalled = plugins.some(
    (p) => p.startsWith("@anyon-cli/anyon"),
  );
  ...
  const { hasOpenAI, hasOpencodeZen, hasZaiCodingPlan, hasKimiForCoding } =
    detectProvidersFromAnyonConfig();
