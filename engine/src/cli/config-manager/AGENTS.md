...
| File                                    | Purpose                                                     |
| --------------------------------------- | ----------------------------------------------------------- |
| `add-plugin-to-opencode-config.ts`      | Register `anyon` in `.opencode/opencode.json` plugin array  |
...
| `generate-anyon-config.ts`              | Generate `anyon.jsonc` from install selections              |
...
| `plugin-name-with-version.ts`           | Resolve `anyon@X.Y.Z` for installation                      |
| `write-anyon-config.ts`                 | Write generated config to `.opencode/anyon.jsonc`           |
...
5. generate-anyon-config (build config from user selections)
6. write-anyon-config
