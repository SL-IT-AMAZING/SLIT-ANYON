...
beforeEach(async () => {
  ...
  fetchVersionSpy = spyOn(module, "fetchLatestVersion").mockResolvedValue(
    "1.2.3",
  );
});
...
describe("addAuthPlugins", () => {
  ...
  "provider": {}
      ...
      expect(
        plugins.some((p) => p.startsWith("opencode-antigravity-auth")),
      ).toBe(false);
    });
  });
  ...
  "provider": {}
      ...
      const antigravityCount = plugins.filter((p) =>
