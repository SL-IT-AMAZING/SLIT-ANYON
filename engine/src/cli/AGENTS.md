...
| `run <message>`     | Non-interactive session launcher  | Agent resolution (flag → env → config → Conductor)    |
...
│   ├── agent-resolver.ts        # Flag → env → config → Conductor
...
Agent-specific: researcher→ZAI, scout→Haiku/nano, craftsman→requires OpenAI/Copilot
