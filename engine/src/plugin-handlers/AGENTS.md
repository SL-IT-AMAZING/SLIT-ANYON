...
| `agent-priority-order.ts`        | ~30   | conductor, craftsman, strategist, taskmaster first |
...
| --------------------- | ------------------------ | ------------------------------------ |
| Researcher            | grep*app*\*              | —                                    |
| Taskmaster, Conductor, Strategist | task, task\_\*, teammate | —                        |
| Craftsman             | task                     | —                                    |
...
```
User (~/.config/opencode/anyon.jsonc)
  ↓ deepMerge
Project (.opencode/anyon.jsonc)
  ↓ Zod defaults
Final Config
```
