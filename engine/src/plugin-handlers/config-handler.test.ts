...
beforeEach(() => {
  spyOn(agents, "createBuiltinAgents" as any).mockResolvedValue({
    conductor: { name: "conductor", prompt: "test", mode: "primary" },
...
describe("Conductor-Junior model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    // #given
    const pluginConfig: AnyonConfig = {};
    ...
    expect(agentConfig[getAgentDisplayName("worker")]?.model).toBe(
      worker.WORKER_DEFAULTS.model,
    );
  });

  test("uses explicitly configured worker model", async () => {
    // #given
    const pluginConfig: AnyonConfig = {
      agents: {
        worker: {
...
describe("Plan agent demote behavior", () => {
  test("orders core agents as conductor -> craftsman -> strategist -> taskmaster", async () => {
    // #given
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      ...
      advisor: { name: "advisor", prompt: "test", mode: "subagent" },
      taskmaster: { name: "taskmaster", prompt: "test", mode: "primary" },
    });
    const pluginConfig: AnyonConfig = {
      conductor_agent: {
    ...
    const coreAgents = [
      ...
      getAgentDisplayName("taskmaster"),
    ];
...
describe("Hidden internal planning agents", () => {
  test("strategist and taskmaster are hidden in founder-facing config", async () => {
    const createBuiltinAgentsMock = agents.createBuiltinAgents as unknown as {
      mockResolvedValue: (value: Record<string, unknown>) => void;
    };
    createBuiltinAgentsMock.mockResolvedValue({
    ...
    expect(agentConfig[getAgentDisplayName("strategist")]?.hidden).toBe(true);
    expect(agentConfig[getAgentDisplayName("taskmaster")]?.hidden).toBe(true);
  });
});
...
describe("default_agent behavior with Conductor orchestration", () => {
  ...
  test("sets default_agent to conductor when missing", async () => {
    // #given
    const pluginConfig: AnyonConfig = {};
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
    ...
    // #then
    expect(config.default_agent).toBe(getAgentDisplayName("conductor"));
  });

  test("sets default_agent to conductor when configured default_agent is empty after trim", async () => {
    // given
    const pluginConfig: AnyonConfig = {};
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
  ...
  test("does not normalize configured default_agent when Conductor is disabled", async () => {
    // given
    const pluginConfig: AnyonConfig = {
      conductor_agent: {
        disabled: true,
...
describe("per-agent todowrite/todoread deny when task_system enabled", () => {
  ...
  test("does not deny todowrite/todoread when task_system is disabled", async () => {
    ...
    >;
    expect(
      agentResult[getAgentDisplayName("conductor")]?.permission?.todowrite,
    ).toBeUndefined();
    expect(
      agentResult[getAgentDisplayName("conductor")]?.permission?.todoread,
    ).toBeUndefined();
    expect(
      agentResult[getAgentDisplayName("craftsman")]?.permission?.todowrite,
    ).toBeUndefined();
    expect(
      agentResult[getAgentDisplayName("craftsman")]?.permission?.todoread,
    ).toBeUndefined();
  });
...
describe("disable_omo_env pass-through", () => {
  test("passes disable_omo_env=true to createBuiltinAgents", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      conductor: { name: "conductor", prompt: "without-env", mode: "primary" },
    });
  ...
  test("passes disable_omo_env=false to createBuiltinAgents when omitted", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      conductor: { name: "conductor", prompt: "with-env", mode: "primary" },
    });
