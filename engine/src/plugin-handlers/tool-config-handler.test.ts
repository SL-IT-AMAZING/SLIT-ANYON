...
function createParams(overrides: { taskSystem?: boolean; agents?: string[] }) {
...
describe("applyToolConfig", () => {
  describe("#given task_system is enabled", () => {
    describe("#when applying tool config", () => {
      ...
      it.each(["taskmaster", "conductor", "craftsman", "strategist", "worker"])(
        "#then should deny todo tools for %s agent",
        (agentName) => {
  ...
  describe("#given task_system is disabled", () => {
    describe("#when applying tool config", () => {
      it.each(["taskmaster", "conductor", "craftsman", "strategist", "worker"])(
        "#then should NOT deny todo tools for %s agent",
        (agentName) => {
          const params = createParams({
            taskSystem: false,
            agents: [agentName],
