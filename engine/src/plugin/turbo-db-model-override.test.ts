...
describe("scheduleDeferredModelOverride", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "turbo-db-test-"));
  ...
  test("should update model in DB after microtask flushes", async () => {
    ...
    //#when
    const { scheduleDeferredModelOverride } =
      await import("./turbo-db-model-override");
  ...
  test("should fall back to setTimeout when message never appears", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ messageId: "msg_nonexistent", attempt: 10 }),
    );
  });
