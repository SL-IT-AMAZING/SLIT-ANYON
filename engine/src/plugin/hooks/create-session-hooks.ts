import type { AnyonConfig, HookName } from "../../config";
...
import {
  ...
  createNoConductorGptHook,
  createNoCraftsmanNonGptHook,
  createNonInteractiveEnvHook,
  createPersistLoopHook,
...
export type SessionHooks = {
  contextWindowMonitor: ReturnType<
    typeof createContextWindowMonitorHook
  > | null;
  preemptiveCompaction: ReturnType<
    typeof createPreemptiveCompactionHook
  > | null;
  ...
  > | null;
  persistLoop: ReturnType<typeof createPersistLoopHook> | null;
  ...
  noConductorGpt: ReturnType<typeof createNoConductorGptHook> | null;
...
export function createSessionHooks(args: {
  ...
  const autoUpdateChecker = isHookEnabled("auto-update-checker")
    ? safeHook("auto-update-checker", () =>
        createAutoUpdateCheckerHook(ctx, {
          ...
          isConductorEnabled: pluginConfig.conductor_agent?.disabled !== true,
  ...
  const persistLoop = isHookEnabled("persist-loop")
    ? safeHook("persist-loop", () =>
        createPersistLoopHook(ctx, {
          config: pluginConfig.persist_loop,
  ...
  const strategistMdOnly = isHookEnabled("strategist-md-only")
  ...
  const noConductorGpt = isHookEnabled("no-conductor-gpt")
    ? safeHook("no-conductor-gpt", () => createNoConductorGptHook(ctx))
    : null;

  const noCraftsmanNonGpt = isHookEnabled("no-craftsman-non-gpt")
    ? safeHook("no-craftsman-non-gpt", () =>
        createNoCraftsmanNonGptHook(ctx, {
          allowNonGptModel:
            pluginConfig.agents?.craftsman?.allow_non_gpt_model,
        }),
      )
    : null;
