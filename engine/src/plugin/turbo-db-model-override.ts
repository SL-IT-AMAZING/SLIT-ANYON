...
function retryViaMicrotask(
  ...
  if (attempt >= MAX_MICROTASK_RETRIES) {
    log(
      "[turbo-db-override] Exhausted microtask retries, falling back to setTimeout",
      {
        messageId,
        attempt,
      },
    ...
    setTimeout(() => {
      try {
        if (tryUpdateMessageModel(db, messageId, targetModel, variant)) {
          log(
            `[turbo-db-override] setTimeout fallback succeeded: ${targetModel.providerID}/${targetModel.modelID}`,
            { messageId },
          );
        } else {
          log(
            "[turbo-db-override] setTimeout fallback failed - message not found",
            { messageId },
          );
        }
      } catch (error) {
        log("[turbo-db-override] setTimeout fallback failed with error", {
          messageId,
          error: String(error),
        });
      } finally {
        try {
          ...
          log(
            "[turbo-db-override] Failed to close DB after setTimeout fallback",
            {
              messageId,
              error: String(error),
            },
  ...
  queueMicrotask(() => {
    ...
    try {
      if (tryUpdateMessageModel(db, messageId, targetModel, variant)) {
        log(
          `[turbo-db-override] Deferred DB update (attempt ${attempt}): ${targetModel.providerID}/${targetModel.modelID}`,
          { messageId },
        );
        return;
      }
      ...
      log("[turbo-db-override] Deferred DB update failed with error", {
        messageId,
        attempt,
        error: String(error),
      });
      ...
      if (shouldCloseDb) {
        try {
          ...
          log(
            "[turbo-db-override] Failed to close DB after deferred DB update",
            {
              messageId,
              attempt,
              error: String(error),
...
export function scheduleDeferredModelOverride(
  ...
  queueMicrotask(() => {
    ...
    if (!existsSync(dbPath)) {
      log("[turbo-db-override] DB not found, skipping deferred override");
      return;
    }
    ...
    try {
      ...
      log("[turbo-db-override] Failed to open DB, skipping deferred override", {
        messageId,
        error: String(error),
      });
      return;
    ...
    try {
      ...
      log("[turbo-db-override] Failed to apply deferred model override", {
        error: String(error),
      });
      db.close();
    }
