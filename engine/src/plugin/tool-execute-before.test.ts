...
describe("createToolExecuteBeforeHandler", () => {
  ...
  describe("task tool subagent_type normalization", () => {
    ...
    test("sets subagent_type to worker when category is provided without subagent_type", async () => {
      //#given
      ...
      //#then
      expect(output.args.subagent_type).toBe("worker");
    });
    ...
    test("sets subagent_type to worker when category provided with different subagent_type", async () => {
      //#given
      const ctx = createCtxWithSessionMessages();
      const handler = createToolExecuteBeforeHandler({
        ctx,
    ...
    test("resolves subagent_type from session first message when session_id provided without subagent_type", async () => {
      ...
      const ctx = createCtxWithSessionMessages([
        ...
        { info: { role: "assistant", agent: "advisor" } },
      ]);
      const handler = createToolExecuteBeforeHandler({
        ctx,
        hooks: emptyHooks,
    ...
    test("does not set subagent_type when neither category nor session_id is provided and subagent_type is present", async () => {
      ...
      //#then
      expect(output.args.subagent_type).toBe("advisor");
    });
  });
});
