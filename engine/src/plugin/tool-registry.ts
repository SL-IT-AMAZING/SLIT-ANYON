...
export function createToolRegistry(args: {
  ...
  const callOmoAgent = createCallAnyonAgent(ctx, managers.backgroundManager, pluginConfig.disabled_agents ?? [])

  const isInspectorEnabled = !(pluginConfig.disabled_agents ?? []).some(
    (agent) => agent.toLowerCase() === "inspector",
  )
  const lookAt = isInspectorEnabled ? createLookAt(ctx) : null
