...
import { scheduleDeferredModelOverride } from "./turbo-db-model-override";

const CODE_BLOCK = /```[\s\S]*?```/g;
const INLINE_CODE = /`[^`]+`/g;
const TURBO_PATTERN = /\b(turbo|ulw)\b/i;

export function detectTurbo(text: string): boolean {
  const clean = text.replace(CODE_BLOCK, "").replace(INLINE_CODE, "");
  return TURBO_PATTERN.test(clean);
}
...
export type TurboOverrideResult = {
  providerID?: string;
  modelID?: string;
  variant?: string;
};
...
/**
 * Resolves the turbo model override config for the given agent and prompt text.
 * Returns null if no override should be applied.
 */
export function resolveTurboOverride(
  ...
  sessionID?: string,
): TurboOverrideResult | null {
  const promptText = extractPromptText(output.parts);
  if (!detectTurbo(promptText)) return null;
  ...
  const turboConfig = agentConfig?.turbo;
  if (!turboConfig?.model && !turboConfig?.variant) return null;

  if (!turboConfig.model) {
    return {
      variant: turboConfig.variant,
    };
  }

  const modelParts = turboConfig.model.split("/");
...
/**
 * Applies turbo model override using a deferred DB update strategy.
 *
 ...
 */
export function applyTurboModelOverrideOnMessage(
  pluginConfig: AnyonConfig,
  inputAgentName: string | undefined,
  output: {
    message: Record<string, unknown>;
  ...
  const override = resolveTurboOverride(
    pluginConfig,
    inputAgentName,
    output,
    sessionID,
  ...
  if (isSameModel(output.message.model, targetModel)) {
    log(
      `[turbo-model-override] Skip override; target model already active: ${override.modelID}`,
    );
    return;
  }
  ...
  if (!messageId) {
    log(
      "[turbo-model-override] No message ID found, falling back to direct mutation",
    );
  ...
  log(
    `[turbo-model-override] ${fromModel} -> ${override.modelID} (deferred DB)`,
    {
