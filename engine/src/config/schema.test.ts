...
describe("disabled_mcps schema", () => {
  test("should accept built-in MCP names", () => {
    ...
    // when
    const result = AnyonConfigSchema.safeParse(config);
...
describe("Conductor-Junior agent override", () => {
  test("schema accepts agents['Conductor-Junior'] and retains the key after parsing", () => {
    // given
    const config = {
      agents: {
        worker: {
    ...
    if (result.success) {
      ...
      expect(result.data.agents?.["worker"]?.model).toBe("openai/gpt-5.2");
  ...
  test("schema accepts lowercase agent names (conductor, taskmaster, strategist)", () => {
    // given
    const config = {
      agents: {
        conductor: {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.agents?.conductor?.temperature).toBe(0.1);
      expect(result.data.agents?.taskmaster?.temperature).toBe(0.2);
      expect(result.data.agents?.strategist?.temperature).toBe(0.3);
    }
  });

  test("schema accepts lowercase analyst and critic agent names", () => {
    // given
    const config = {
      agents: {
        analyst: {
...
describe("AnyonConfigSchema - browser_automation_engine", () => {
  test("accepts browser_automation_engine config", () => {
    ...
    // when
    const result = AnyonConfigSchema.safeParse(input);
...
describe("AnyonConfigSchema - hashline_edit", () => {
