...
describe("look_at tool conditional registration", () => {
  describe("isInspectorEnabled logic", () => {
    // given inspector is in disabled_agents
    // when checking if agent is enabled
    // then should return false (disabled)
    it("returns false when inspector is disabled (exact case)", () => {
      const disabledAgents: string[] = ["inspector"];
      const isEnabled = !disabledAgents.some(
        (agent) => agent.toLowerCase() === "inspector",
      );
      expect(isEnabled).toBe(false);
    });

    // given inspector is in disabled_agents with different case
    // when checking if agent is enabled
    // then should return false (case-insensitive match)
    it("returns false when inspector is disabled (case-insensitive)", () => {
      const disabledAgents: string[] = ["Inspector"];
      const isEnabled = !disabledAgents.some(
        (agent) => agent.toLowerCase() === "inspector",
      );
    ...
    // given inspector is NOT in disabled_agents
    // when checking if agent is enabled
    // then should return true (enabled)
    it("returns true when inspector is not disabled", () => {
