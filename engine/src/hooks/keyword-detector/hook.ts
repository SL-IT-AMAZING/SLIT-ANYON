  ...
  return {
    "chat.message": async (
      ...
      if (isPlannerAgent(currentAgent)) {
        detectedKeywords = detectedKeywords.filter((k) => k.type !== "turbo");
      }
      ...
      if (isNonMainSession) {
        ...
        if (detectedKeywords.length === 0) {
          log(
            `[keyword-detector] Skipping non-turbo keywords in non-main session`,
            {
              sessionID: input.sessionID,
              mainSessionID,
            },
      ...
      const hasTurbo = detectedKeywords.some((k) => k.type === "turbo");
      if (hasTurbo) {
        log(`[keyword-detector] Turbo mode activated`, {
          sessionID: input.sessionID,
        });
