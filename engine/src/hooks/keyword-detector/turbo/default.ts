/**
 * Default turbo message optimized for Claude series models.
 *
 * Key characteristics:
 * - Natural tool-like usage of scout/researcher agents (run_in_background=true)
 * - Parallel execution emphasized - fire agents and continue working
 * - Simple workflow: EXPLORES → GATHER → PLAN → DELEGATE
 */

export const TURBO_DEFAULT_MESSAGE = `<turbo-mode>
...
| **SCOUT** the codebase to understand existing patterns, architecture, and context |
...
2. **SCOUT THOROUGHLY** - Fire scout/researcher agents to gather ALL relevant context
...
task(subagent_type="advisor", load_skills=[], prompt="I need architectural review of my approach to [TASK]. Here's my plan: [DESCRIBE PLAN WITH SPECIFIC FILES AND CHANGES]. My concerns are: [LIST SPECIFIC UNCERTAINTIES]. Please evaluate: correctness of approach, potential issues I'm missing, and whether a better alternative exists.", run_in_background=false)
\`\`\`
...
5. **DO** scout alternative approaches
...
|-----------|--------|-----|
| Codebase exploration | task(subagent_type="scout", load_skills=[], run_in_background=true) | Parallel, context-efficient |
...
| Hard problem (conventional) | task(subagent_type="advisor", load_skills=[]) | Architecture, debugging, complex logic |
...
export function getDefaultTurboMessage(): string {
  return TURBO_DEFAULT_MESSAGE;
}
