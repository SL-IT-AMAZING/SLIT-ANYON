...
import type { ExperimentalConfig, AnyonConfig } from "../../config";
...
export function createAnthropicContextWindowLimitRecoveryHook(
  ...
  const eventHandler = async ({
    ...
    if (event.type === "session.error") {
      ...
      log("[auto-compact] session.error received", {
        sessionID,
        error: props?.error,
      });
      if (!sessionID) return;
      ...
      if (parsed) {
        ...
        const lastAssistant = await getLastAssistant(
          sessionID,
          ctx.client,
          ctx.directory,
        );
        const providerID =
          parsed.providerID ??
          (lastAssistant?.providerID as string | undefined);
        const modelID =
          parsed.modelID ?? (lastAssistant?.modelID as string | undefined);
    ...
    if (event.type === "message.updated") {
      ...
      if (sessionID && info?.role === "assistant" && info.error) {
        log("[auto-compact] message.updated with error", {
          sessionID,
          error: info.error,
        });
    ...
    if (event.type === "session.idle") {
      ...
      const providerID =
        ...
        (lastAssistant?.providerID as string | undefined);
      const modelID =
        errorData?.modelID ?? (lastAssistant?.modelID as string | undefined);
