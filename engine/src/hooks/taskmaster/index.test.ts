...
import {
  ...
  clearThesisState,
  readThesisState,
} from "../../features/thesis-state";
import type { ThesisState } from "../../features/thesis-state";
import {
  _resetForTesting,
  subagentSessions,
  updateSessionAgent,
...
const TEST_STORAGE_ROOT = join(
  tmpdir(),
  `taskmaster-message-storage-${randomUUID()}`,
);
...
const { createTaskmasterHook } = await import("./index");
const { MESSAGE_STORAGE } =
  await import("../../features/hook-message-injector");

describe("taskmaster hook", () => {
  let TEST_DIR: string;
  let EULER_DIR: string;
  ...
  function createMockPluginInput(overrides?: {
    ...
    } as unknown as Parameters<typeof createTaskmasterHook>[0] & {
  ...
  beforeEach(() => {
    TEST_DIR = join(tmpdir(), `taskmaster-test-${randomUUID()}`);
    EULER_DIR = join(TEST_DIR, ".anyon");
  ...
  describe("tool.execute.after handler", () => {
    test("should handle undefined output gracefully (issue #1035)", async () => {
      ...
      const hook = createTaskmasterHook(createMockPluginInput());
    ...
    test("should not transform when caller is not Taskmaster", async () => {
      // given - thesis state exists but caller agent in message storage is not Taskmaster
    ...
    test("should append standalone verification when no thesis state but caller is Taskmaster", async () => {
      // given - no thesis state, but caller is Taskmaster
      const sessionID = "session-no-thesis-test";
      setupMessageStorage(sessionID, "taskmaster");
    ...
    test("should transform output when caller is Taskmaster with thesis state", async () => {
      // given - Taskmaster caller with thesis state
    ...
    test("should still transform when plan is complete (shows progress)", async () => {
      // given - thesis state with complete plan, Taskmaster caller
    ...
    test("should append session ID to thesis state if not present", async () => {
      // given - thesis state without session-append-test, Taskmaster caller
      ...
      const updatedState = readThesisState(TEST_DIR);
    ...
    test("should not duplicate existing session ID", async () => {
      // given - thesis state already has session-dup-test, Taskmaster caller
    ...
    test("should include thesis.json path and notepad path in transformed output", async () => {
      // given - thesis state, Taskmaster caller
    ...
    describe("Write/Edit tool direct work reminder", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(ORCHESTRATOR_SESSION, "taskmaster");
      });
      ...
      test("should append delegation reminder when orchestrator writes outside .anyon/", async () => {
        // given
        const hook = createTaskmasterHook(createMockPluginInput());
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .anyon/", async () => {
        // given
        const hook = createTaskmasterHook(createMockPluginInput());
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .anyon/", async () => {
        // given
        const hook = createTaskmasterHook(createMockPluginInput());
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.anyon/plans/work-plan.md" },
        };
      ...
      test("should NOT append reminder when non-orchestrator writes outside .anyon/", async () => {
        // given
        const nonOrchestratorSession = "non-orchestrator-session";
        setupMessageStorage(nonOrchestratorSession, "worker");
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .anyon\\ (Windows backslash)", async () => {
          // given
          const hook = createTaskmasterHook(createMockPluginInput());
          const originalOutput = "File written successfully";
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".anyon\\plans\\work-plan.md" },
          };
        ...
        test("should NOT append reminder when orchestrator writes inside .anyon with mixed separators", async () => {
          // given
          const hook = createTaskmasterHook(createMockPluginInput());
          const originalOutput = "File written successfully";
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .anyon\\", async () => {
          // given
          const hook = createTaskmasterHook(createMockPluginInput());
          const originalOutput = "File written successfully";
          const output = {
            ...
            output: originalOutput,
            metadata: {
              filePath: "C:\\Users\\test\\project\\.anyon\\plans\\x.md",
            },
          };
        ...
        test("should append reminder for Windows path outside .anyon\\", async () => {
          // given
          const hook = createTaskmasterHook(createMockPluginInput());
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (thesis continuation)", () => {
    ...
    beforeEach(() => {
      ...
      setupMessageStorage(MAIN_SESSION_ID, "taskmaster");
    });
    ...
    test("should inject continuation when thesis has incomplete tasks", async () => {
      // given - thesis state with incomplete plan
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(
        planPath,
        "# Plan\n- [ ] Task 1\n- [x] Task 2\n- [ ] Task 3",
      ...
      const hook = createTaskmasterHook(mockInput);
    ...
    test("should not inject when no thesis state exists", async () => {
      // given - no thesis state
      const mockInput = createMockPluginInput();
      const hook = createTaskmasterHook(mockInput);
    ...
    test("should not inject when main session is not in thesis session_ids", async () => {
      // given - thesis state exists but current (main) session is NOT in session_ids
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
      ...
      // when - main session fires idle but is NOT in thesis's session_ids
      await hook.handler({
        event: {
          type: "session.idle",
          properties: { sessionID: MAIN_SESSION_ID },
      ...
      // then - should NOT call prompt because session is not part of this thesis
      expect(mockInput._promptMock).not.toHaveBeenCalled();
    });

    test("should not inject when thesis plan is complete", async () => {
      // given - thesis state with complete plan
      const planPath = join(TEST_DIR, "complete-plan.md");
      writeFileSync(planPath, "# Plan\n- [x] Task 1\n- [x] Task 2");
    ...
    test("should clear abort state on message.updated", async () => {
      // given - thesis with incomplete plan
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1");
    ...
    test("should include plan progress in continuation prompt", async () => {
      // given - thesis state with specific progress
    ...
    test("should inject when last agent is conductor and thesis targets taskmaster explicitly", async () => {
      // given - thesis explicitly set to taskmaster, but last agent is conductor (initial state after /start-work)
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
      ...
      // given - last agent is conductor (typical state right after /start-work)
      cleanupMessageStorage(MAIN_SESSION_ID);
      setupMessageStorage(MAIN_SESSION_ID, "conductor");
      ...
      // then - should call prompt because conductor is always allowed for taskmaster thesiss
      expect(mockInput._promptMock).toHaveBeenCalled();
    });

    test("should not inject when last agent is non-conductor and does not match thesis agent", async () => {
      // given - thesis explicitly set to taskmaster, last agent is craftsman (unrelated agent)
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
      ...
      // then - should NOT call prompt because craftsman does not match taskmaster or conductor
      expect(mockInput._promptMock).not.toHaveBeenCalled();
    });

    test("should inject when last agent matches thesis agent even if non-Taskmaster", async () => {
      // given - thesis state expects conductor and last agent is conductor
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
      ...
      // then - should call prompt for conductor
      expect(mockInput._promptMock).toHaveBeenCalled();
      const callArgs = mockInput._promptMock.mock.calls[0][0];
      expect(callArgs.body.agent).toBe("conductor");
    });
    ...
    test("should stop continuation after 2 consecutive prompt failures (issue #1355)", async () => {
      //#given - thesis state with incomplete plan and prompt always fails
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
    ...
    test("should reset prompt failure counter after successful retry beyond backoff window", async () => {
      //#given - thesis state with incomplete plan and success on first retry after backoff
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
    ...
    test("should cleanup on session.deleted", async () => {
      ...
      // Re-create thesis after deletion
      writeThesisState(TEST_DIR, state);
    ...
    test("should inject when session agent was updated to taskmaster by start-work even if message storage agent differs", async () => {
      // given - thesis targets taskmaster, but nearest stored message still says craftsman
      const planPath = join(TEST_DIR, "test-plan.md");
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2");
      ...
      cleanupMessageStorage(MAIN_SESSION_ID);
      setupMessageStorage(MAIN_SESSION_ID, "craftsman");
      updateSessionAgent(MAIN_SESSION_ID, "taskmaster");
      ...
      // then - should continue because start-work updated session agent to taskmaster
      expect(mockInput._promptMock).toHaveBeenCalled();
    });
  });
});
