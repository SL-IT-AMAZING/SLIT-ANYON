...
import type { RuntimeFallbackConfig, AnyonConfig } from "../../config";
    ...
    test("should trigger fallback on Copilot auto-retry signal in message.updated", async () => {
      ...
      await hook.event({
        event: {
          type: "session.created",
          properties: {
            info: { id: sessionID, model: "github-copilot/claude-opus-4.6" },
          },
        },
      });
      ...
      expect(fallbackLog).toBeDefined();
      expect(fallbackLog?.data).toMatchObject({
        from: "github-copilot/claude-opus-4.6",
        to: "openai/gpt-5.2",
      });
    });
  ...
  describe("fallback models configuration", () => {
    ...
    test("should use agent-level fallback_models", async () => {
      ...
      const hook = createRuntimeFallbackHook(input, {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("advisor", [
          "openai/gpt-5.2",
          "google/gemini-3.1-pro",
        ]),
      });
    ...
    test("should detect agent from sessionID pattern", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("conductor", [
          "openai/gpt-5.2",
        ]),
      });
      const sessionID = "conductor-session-123";
      ...
      //#then - should detect conductor from sessionID and use its fallback
      const fallbackLog = logCalls.find((c) =>
        c.msg.includes("Preparing fallback"),
      );
      expect(fallbackLog).toBeDefined();
    ...
    test("should preserve resolved agent during auto-retry", async () => {
      ...
      const hook = createRuntimeFallbackHook(
        ...
        {
          config: createMockConfig({ notify_on_fallback: false }),
          pluginConfig: createMockPluginConfigWithAgentFallback("strategist", [
            "github-copilot/claude-opus-4.6",
          ]),
        },
      );
