...
describe("model fallback hook", () => {
  ...
  test("preserves fallback progression across repeated session.error retries", async () => {
    ...
    expect(
      setPendingModelFallback(sessionID, "Conductor (Turbo)", "anthropic", "claude-opus-4-6-thinking"),
    ).toBe(true)
    ...
    expect(
      setPendingModelFallback(sessionID, "Conductor (Turbo)", "anthropic", "claude-opus-4-6"),
    ).toBe(true)
