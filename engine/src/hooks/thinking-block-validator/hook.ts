  ...
  for (let i = currentIndex - 1; i >= 0; i--) {
    ...
    for (const part of msg.parts) {
      ...
      if (type === "thinking" || type === "reasoning") {
        const thinking =
          (part as unknown as ThinkingPart).thinking ||
          (part as unknown as ThinkingPart).text;
        if (
          thinking &&
...
export function createThinkingBlockValidatorHook(): MessagesTransformHook {
  return {
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      const modelID =
        (lastUserMessage?.info as unknown as MessageInfoExtended)?.modelID ||
        "";
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (hasContentParts(msg.parts) && !startsWithThinkingBlock(msg.parts)) {
          ...
          const thinkingContent =
            previousThinking || "[Continuing from previous reasoning]";
