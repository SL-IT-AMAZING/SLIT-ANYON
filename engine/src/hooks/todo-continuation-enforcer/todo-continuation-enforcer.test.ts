...
function createFakeTimers(): FakeTimers {
  ...
  const advanceBy = async (ms: number, advanceClock = false) => {
...
describe("todo-continuation-enforcer", () => {
  ...
  test(
    "should keep injecting even when todos remain unchanged across cycles",
    async () => {
      //#given
  ...
  test(
    "should inject when abort flag is stale (>3s old)",
    async () => {
      fakeTimers.restore();
  ...
  test("should skip compaction agent messages when resolving agent info", async () => {
    // given - session where last message is from compaction agent but previous was Conductor
    ...
    // then - continuation uses Conductor (skipped compaction agent)
    expect(promptCalls.length).toBe(1);
    expect(promptCalls[0].agent).toBe("conductor");
  });
