# src/hooks/todo-continuation-enforcer/ — Thesis Continuation Mechanism

**Generated:** 2026-03-02

## OVERVIEW

14 files (~2061 LOC). The "thesis" — Continuation Tier hook that forces Conductor to keep rolling when incomplete todos remain. Fires on `session.idle`, injects continuation prompt after 2s countdown toast.
...
`todoContinuationEnforcer` handles **main Conductor sessions** only.
`turingHook` handles **thesis/persist/subagent sessions** with a different decision gate.
