...
/**
 * Get the cache directory for anyon binaries.
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[anyon] Downloading comment-checker binary...`);
  ...
  try {
    ...
    log(`[anyon] comment-checker binary ready.`);

    return binaryPath;
  } catch (err) {
    debugLog(`Failed to download: ${err}`);
    log(
      `[anyon] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`,
    );
    log(`[anyon] Comment checking disabled.`);
    return null;
  }
}
