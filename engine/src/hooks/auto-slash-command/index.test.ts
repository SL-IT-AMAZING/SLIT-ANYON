...
describe("createAutoSlashCommandHook", () => {
  ...
  describe("excluded commands", () => {
    it("should NOT trigger for persist-loop command", async () => {
      // given persist-loop command
      const hook = createAutoSlashCommandHook();
      const sessionID = `test-session-persist-${Date.now()}`;
      const input = createMockInput(sessionID);
      const output = createMockOutput("/persist-loop do something");
      const originalText = output.parts[0].text;
    ...
    it("should NOT trigger for cancel-persist command", async () => {
      // given cancel-persist command
      const hook = createAutoSlashCommandHook();
      const sessionID = `test-session-cancel-${Date.now()}`;
      const input = createMockInput(sessionID);
      const output = createMockOutput("/cancel-persist");
      const originalText = output.parts[0].text;
  ...
  describe("command.execute.before hook", () => {
    ...
    it("should inject template for known builtin commands like persist-loop", async () => {
      //#given
      const hook = createAutoSlashCommandHook();
      const input = createCommandInput("persist-loop");
      ...
      expect(output.parts[0].text).toContain("/persist-loop Command");
    });
