...
3. Update \`.anyon/thesis.json\` — add \`"worktree_path": "<absolute-path>"\`
...
function resolveWorktreeContext(explicitWorktreePath: string | null): {
  ...
  return {
    ...
    block: `\n**Worktree** (needs setup): \`git worktree add ${explicitWorktreePath} <branch>\`, then add \`"worktree_path"\` to thesis.json`,
  };
}
...
export function createStartWorkHook(ctx: PluginInput) {
  return {
    "chat.message": async (
      ...
      updateSessionAgent(input.sessionID, "taskmaster");

      const existingState = readThesisState(ctx.directory);
      ...
      if (explicitPlanName) {
        log(
          `[${HOOK_NAME}] Explicit plan name requested: ${explicitPlanName}`,
          { sessionID: input.sessionID },
        );
...
All ${progress.total} tasks are done. Create a new plan with: /plan "your task"`;
            ...
            if (existingState) clearThesisState(ctx.directory);
            const newState = createThesisState(
              matchedPlan,
              sessionId,
              "taskmaster",
              worktreePath,
            );
            writeThesisState(ctx.directory, newState);
...
thesis.json has been created. Read the plan and begin execution.`;
          }
        } else {
          const incompletePlans = allPlans.filter(
            (p) => !getPlanProgress(p).isComplete,
          );
...
No incomplete plans available. Create a new plan with: /plan "your task"`;
        ...
        if (!progress.isComplete) {
          ...
          if (worktreePath !== undefined) {
            const updatedSessions = existingState.session_ids.includes(
              sessionId,
            )
              ? existingState.session_ids
              : [...existingState.session_ids, sessionId];
            writeThesisState(ctx.directory, {
              ...existingState,
...
No Strategist plan files found at .anyon/plans/
