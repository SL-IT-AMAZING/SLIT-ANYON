...
describe("parseUserRequest", () => {
  ...
  describe("when turbo keywords are present", () => {
    test("#given plan name with turbo keyword #when parsing #then strips keyword from plan name", () => {
      const result = parseUserRequest(
        "<user-request>my-plan turbo</user-request>",
      );
      expect(result.planName).toBe("my-plan");
    });
    ...
    test("#given only turbo keyword with worktree #when parsing #then plan name is null, worktree preserved", () => {
      const result = parseUserRequest(
        "<user-request>turbo --worktree /wt</user-request>",
      );
      expect(result.planName).toBeNull();
