...
import { createPersistLoopHook } from "./index";
import { readState, writeState, clearState } from "./storage";
import type { PersistLoopState } from "./types";
import { parsePersistLoopArguments } from "./command-arguments";

describe("persist-loop", () => {
  const TEST_DIR = join(tmpdir(), "persist-loop-test-" + Date.now());
  ...
  function createMockPluginInput() {
    return {
      ...
      directory: TEST_DIR,
    } as unknown as Parameters<typeof createPersistLoopHook>[0];
  }
  ...
  describe("storage", () => {
    ...
    test("should handle turbo field", () => {
      // given - a state object with turbo enabled
      const state: PersistLoopState = {
        active: true,
        iteration: 1,
        max_iterations: 50,
      ...
      // then - turbo field should be preserved
      expect(readResult?.turbo).toBe(true);
    });
  ...
  describe("command arguments", () => {
    test("should parse --strategy=reset flag", () => {
      // given - persist-loop command arguments with reset strategy
      const rawArguments =
        '"Build feature X" --strategy=reset --max-iterations=12';

      // when - parse command arguments
      const parsedArguments = parsePersistLoopArguments(rawArguments);
    ...
    test("should parse --strategy=continue flag", () => {
      // given - persist-loop command arguments with continue strategy
  ...
  describe("hook", () => {
    test("should start loop and write state", () => {
      // given - hook instance
      const hook = createPersistLoopHook(createMockPluginInput());
    ...
    test("should accept turbo option in startLoop", () => {
      // given - hook instance
      const hook = createPersistLoopHook(createMockPluginInput());

      // when - start loop with turbo
      hook.startLoop("session-123", "Build something", { turbo: true });

      // then - state should have turbo=true
      const state = hook.getState();
      expect(state?.turbo).toBe(true);
    });

    test("should handle missing turbo option in startLoop", () => {
      // given - hook instance
      const hook = createPersistLoopHook(createMockPluginInput());

      // when - start loop without turbo
      hook.startLoop("session-123", "Build something");

      // then - state should have turbo=undefined
      const state = hook.getState();
      expect(state?.turbo).toBeUndefined();
    });
    ...
    test("should inject continuation when loop active and no completion detected", async () => {
      ...
      expect(promptCalls[0].text).toContain("PERSIST LOOP");
    ...
    test("should stop loop when max iterations reached", async () => {
      ...
      expect(toastCalls[0].title).toBe("Persist Loop Stopped");
    ...
    test("should detect completion promise and stop loop", async () => {
      ...
      expect(promptCalls.length).toBe(0);
      expect(toastCalls.some((t) => t.title === "Persist Loop Complete!")).toBe(
        true,
      );
      expect(hook.getState()).toBeNull();
    });
    ...
    test("should skip concurrent idle events for same session when handler is in flight", async () => {
      ...
      const hook = createPersistLoopHook(
        mockInput as Parameters<typeof createPersistLoopHook>[0],
      );
    ...
    test("should NOT detect completion from user message in transcript (issue #622)", async () => {
      ...
      // This reproduces the bug where the PERSIST_LOOP_TEMPLATE instructional text
      // containing `<promise>DONE</promise>` is recorded as a user message and
      // falsely triggers completion detection
      const transcriptPath = join(TEST_DIR, "transcript.jsonl");
      const templateText = `You are starting a Persist Loop...
Output <promise>DONE</promise> when fully complete`;
    ...
    test("should NOT detect completion from continuation prompt in transcript (issue #622)", async () => {
      ...
      const transcriptPath = join(TEST_DIR, "transcript.jsonl");
      const continuationText = `PERSIST LOOP 2/100
...
Original task: Build something`;
    ...
    test("should show turbo completion toast", async () => {
      // given - hook with turbo mode and completion in transcript
      const transcriptPath = join(TEST_DIR, "transcript.jsonl");
      const hook = createPersistLoopHook(createMockPluginInput(), {
        getTranscriptPath: () => transcriptPath,
      });
      ...
      hook.startLoop("test-id", "Build API", { turbo: true });
      ...
      // then - turbo toast shown
      const completionToast = toastCalls.find(
        (t) => t.title === "TURBO LOOP COMPLETE!",
      );
      expect(completionToast).toBeDefined();
      expect(completionToast!.message).toMatch(/JUST ULW ULW!/);
    });

    test("should show regular completion toast when turbo disabled", async () => {
      // given - hook without turbo
      const transcriptPath = join(TEST_DIR, "transcript.jsonl");
      const hook = createPersistLoopHook(createMockPluginInput(), {
        getTranscriptPath: () => transcriptPath,
      });
    ...
    test("should prepend turbo to continuation prompt when turbo=true", async () => {
      // given - hook with turbo mode enabled
      const hook = createPersistLoopHook(createMockPluginInput());
      hook.startLoop("session-123", "Build API", { turbo: true });
      ...
      // then - prompt should start with "turbo "
      expect(promptCalls.length).toBe(1);
      expect(promptCalls[0].text).toMatch(/^turbo /);
    });

    test("should NOT prepend turbo to continuation prompt when turbo=false", async () => {
      // given - hook without turbo mode
      const hook = createPersistLoopHook(createMockPluginInput());
      ...
      // then - prompt should NOT start with "turbo "
      expect(promptCalls.length).toBe(1);
      expect(promptCalls[0].text).not.toMatch(/^turbo /);
    });
  });
  ...
  describe("API timeout protection", () => {
    test("should not hang when session.messages() throws", async () => {
      ...
      const hook = createPersistLoopHook(errorMock as any, {
        getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
        apiTimeout: 100,
      });
      hook.startLoop("session-123", "Build something");
