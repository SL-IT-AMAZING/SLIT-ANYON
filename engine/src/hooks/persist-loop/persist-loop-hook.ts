...
import { createPersistLoopEventHandler } from "./persist-loop-event-handler";

export interface PersistLoopHook {
  event: (input: {
...
export function createPersistLoopHook(
  ctx: PluginInput,
  options?: PersistLoopOptions,
): PersistLoopHook {
  ...
  const event = createPersistLoopEventHandler(ctx, {
    directory: ctx.directory,
  ...
  return {
    ...
    getState: loopState.getState as () => PersistLoopState | null,
  };
}
