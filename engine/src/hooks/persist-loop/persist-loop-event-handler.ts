...
import type { PersistLoopOptions, PersistLoopState } from "./types";
...
type LoopStateController = {
  ...
  clear: () => boolean;
  incrementIteration: () => PersistLoopState | null;
  setSessionID: (sessionID: string) => PersistLoopState | null;
};
type PersistLoopEventHandlerOptions = {
  directory: string;
  apiTimeoutMs: number;
  getTranscriptPath: (sessionID: string) => string | undefined;
  checkSessionExists?: PersistLoopOptions["checkSessionExists"];
  sessionRecovery: SessionRecovery;
  loopState: LoopStateController;
};

export function createPersistLoopEventHandler(
  ctx: PluginInput,
  options: PersistLoopEventHandlerOptions,
) {
    ...
    if (event.type === "session.idle") {
      ...
      try {
        ...
        if (completionViaTranscript || completionViaApi) {
          ...
          const title = state.turbo
            ? "TURBO LOOP COMPLETE!"
            : "Persist Loop Complete!";
          const message = state.turbo
