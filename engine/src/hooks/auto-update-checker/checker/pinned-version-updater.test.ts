...
describe("pinned-version-updater", () => {
  ...
  describe("updatePinnedVersion", () => {
    test("updates pinned version in config", () => {
      ...
      expect(updated).toContain("@anyon-cli/anyon@3.4.0");
      expect(updated).not.toContain("@anyon-cli/anyon@3.1.8");
    });
  ...
  describe("revertPinnedVersion", () => {
    test("reverts from failed version back to original entry", () => {
      ...
      expect(reverted).toContain("@anyon-cli/anyon@3.1.8");
      expect(reverted).not.toContain("@anyon-cli/anyon@3.4.0");
    });
    ...
    test("reverts to unpinned entry", () => {
      ...
      expect(result).toBe(true);
      const reverted = fs.readFileSync(configPath, "utf-8");
      expect(reverted).toContain('"@anyon-cli/anyon"');
      expect(reverted).not.toContain("@anyon-cli/anyon@3.4.0");
    });
  ...
  describe("update then revert roundtrip", () => {
    test("config returns to original state after update + revert", () => {
      ...
      //#when
      updatePinnedVersion(configPath, "@anyon-cli/anyon@3.1.8", "3.4.0");
      revertPinnedVersion(configPath, "3.4.0", "@anyon-cli/anyon@3.1.8");

      //#then
      const finalConfig = fs.readFileSync(configPath, "utf-8");
      expect(finalConfig).toContain("@anyon-cli/anyon@3.1.8");
      expect(finalConfig).not.toContain("@anyon-cli/anyon@3.4.0");
    });
  });
});
