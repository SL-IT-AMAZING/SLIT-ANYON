...
export function createAutoUpdateCheckerHook(
  ...
  const getToastMessage = (
    ...
    if (isConductorEnabled) {
      return isUpdate
        ? `Conductor on steroids is steering OpenCode.\nv${latestVersion} available. Restart to apply.`
        : "Conductor on steroids is steering OpenCode.";
    }
    return isUpdate
  ...
  return {
    event: ({ event }: { event: { type: string; properties?: unknown } }) => {
      ...
      setTimeout(async () => {
        ...
        if (localDevVersion) {
          if (showStartupToast) {
            showLocalDevToast(ctx, displayVersion, isConductorEnabled).catch(
              () => {},
            );
          }
        ...
        runBackgroundUpdateCheck(ctx, autoUpdate, getToastMessage).catch(
          (err) => {
