...
export const NEWTON_PLAN_TEMPLATE = `## Planning Artifact Structure

Generate three linked artifacts:

- Founder brief: \`.anyon/briefs/{name}.md\`
- Internal build spec: \`.anyon/specs/{name}.md\`
- Wave 1 execution plan: \`.anyon/plans/{name}.md\`
...
### User Flow Inventory
- [Flow name]: [who it is for] -> [goal] -> [success outcome]
...
### User Flow Coverage
- [Flow name] -> [what this build wave must make possible]
...
- [Explicit exclusion from Analyst review]
...
Evidence saved to \`.anyon/evidence/task-{N}-{scenario-slug}.{ext}\`.
...
\`\`\`
Wave 1 (Approved MVP wave — first user-visible end-to-end slice):
...
- **FINAL**: **4** — F1 → \`advisor\`, F2 → \`unspecified-high\`, F3 → \`unspecified-high\`, F4 → \`deep\`
...
- [ ] 1. [Task Title]
  ...
  **QA Scenarios (MANDATORY — task is INCOMPLETE without these):**
  ...
  Scenario: [Happy path — what SHOULD work]
    ...
    Evidence: .anyon/evidence/task-{N}-{scenario-slug}.{ext}
  ...
  Scenario: [Failure/edge case — what SHOULD fail gracefully]
    ...
    Evidence: .anyon/evidence/task-{N}-{scenario-slug}-error.{ext}
  \\\`\\\`\\\`
...
- [ ] F1. **Plan Compliance Audit** — \`advisor\`
  Read the plan end-to-end. For each "Must Have": verify implementation exists (read file, curl endpoint, run command). For each "Must NOT Have": search codebase for forbidden patterns — reject with file:line if found. Check evidence files exist in .anyon/evidence/. Compare deliverables against plan.
...
- [ ] F3. **Real Manual QA** — \`unspecified-high\` (+ \`playwright\` skill if UI)
  Start from clean state. Execute EVERY QA scenario from EVERY task — follow exact steps, capture evidence. Test cross-task integration (features working together, not isolation). Test edge cases: empty state, invalid input, rapid actions. Save to \`.anyon/evidence/final-qa/\`.
