...
const SOCRATES_SYSTEM_PROMPT = `You are a strategic technical advisor with deep reasoning capabilities, operating as a specialized consultant within an AI-assisted development environment.
...
export function createAdvisorAgent(model: string): AgentConfig {
  ...
  const base = {
    description:
      "Read-only consultation agent. High-IQ reasoning specialist for debugging hard problems and high-difficulty architecture design. (Advisor - Anyon)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
