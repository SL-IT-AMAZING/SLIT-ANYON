...
import { LOOK_AT_DESCRIPTION, INSPECTOR_AGENT } from "./constants"
...
import { resolveInspectorAgentMetadata } from "./multimodal-agent-metadata"
import {
  needsConversion,
  convertImageToJpeg,
...
Original error: ${createResult.error}`
      ...
      const { agentModel, agentVariant } = await resolveInspectorAgentMetadata(ctx)
      ...
      if (!responseText) {
        ...
        return "Error: No response from inspector agent"
      }
