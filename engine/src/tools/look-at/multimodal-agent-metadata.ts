...
import { INSPECTOR_AGENT } from "./constants";
...
export async function resolveInspectorAgentMetadata(
  ctx: PluginInput,
  ...
  try {
    ...
    const matched = agents.find(
      (agent) => agent?.name?.toLowerCase() === INSPECTOR_AGENT.toLowerCase(),
    );
    ...
    log("[look_at] Failed to resolve inspector model info", error);
    return {};
  }
}
