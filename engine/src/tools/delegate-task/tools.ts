...
export function createDelegateTask(
  ...
  **CORRECT - Using subagent_type:**
  ...
  - category: For task delegation (uses Conductor-Junior with category-optimized model)
  - subagent_type: For direct agent invocation (scout, researcher, advisor, etc.)
  
  **DO NOT provide both.** If category is provided, subagent_type is ignored.
  
  - load_skills: ALWAYS REQUIRED. Pass [] if no skills needed, or ["skill-1", "skill-2"] for category tasks.
  - category: Use predefined category → Spawns Conductor-Junior with category config
    Available categories:
  ${categoryList}
  - subagent_type: Use specific agent directly (scout, researcher, advisor, analyst, critic)
  ...
  **WHEN TO USE session_id:**
  ...
  return tool({
    ...
    async execute(args: DelegateTaskArgs, toolContext) {
      ...
      if (args.category) {
        if (args.subagent_type && args.subagent_type !== WORKER_AGENT) {
          log(
            "[task] category provided - overriding subagent_type to worker",
            {
        ...
        args.subagent_type = WORKER_AGENT;
      }
      await ctx.metadata?.({
