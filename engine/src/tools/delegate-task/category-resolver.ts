...
2. Or configure an alternative model in your anyon.json for this category
...
2. Anyon: Set category model in anyon.json
...
Available categories: ${categoryNames.join(", ")}`,
  ...
  const isUnstableAgent =
    ...
    [unstableModel, categoryConfigModel].some((m) =>
      m
        ? m.includes("gemini") || m.includes("minimax") || m.includes("kimi")
        : false,
    );
