...
describe("conductor-task", () => {
  ...
  describe("isPlanFamily", () => {
    ...
    test("returns false for 'advisor'", () => {
      //#given / #when
      const result = isPlanFamily("advisor")
      //#then
      expect(result).toBe(false)
    })
  ...
  describe("category delegation config validation", () => {
    test("fills subagent_type as worker when category is provided without subagent_type", async () => {
      // given
      const { createDelegateTask } = require("./tools")
      ...
      const mockManager = {
       ...
       // then
       expect(args.subagent_type).toBe("Worker")
    }, { timeout: 10000 })

    test("category overrides subagent_type and still maps to worker", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       const mockManager = { launch: async () => ({ id: "task-123", status: "pending", description: "Test task", agent: "worker", sessionID: "test-session" }) }
       const mockClient = {
         app: { agents: async () => ({ data: [] }) },
       ...
       expect(result).not.toContain("anyon requires a default model")
    }, { timeout: 10000 })
  ...
  test("sync continuation preserves variant from previous session message", async () => {
    ...
    const tool = createDelegateTask({
      manager: { resume: async () => ({ id: "task-var", sessionID: "ses_var_test", description: "Variant test", agent: "worker", status: "running" }) },
      client: mockClient,
    })
    ...
    expect(callArgs.body.agent).toBe("worker")
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO workerModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
    ...
    test("worker model override takes precedence over category model", async () => {
      // given - worker override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: any
    ...
    test("explicit category model takes precedence over worker model", async () => {
      // given - explicit category model differs from worker override
      const { createDelegateTask } = require("./tools")
      let launchInput: any
    ...
    test("worker model override works with quick category (#1295)", async () => {
      // given - user configures agents.worker.model but uses quick category
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // then - worker override model should be used, not category default
      expect(launchInput.model.providerID).toBe("anthropic")
      expect(launchInput.model.modelID).toBe("claude-sonnet-4-6")
    })

    test("worker model override works with user-defined category (#1295)", async () => {
      ...
      // then - worker override model should be used as fallback
      expect(launchInput.model.providerID).toBe("openai")
  ...
  describe("plan family mutual delegation block", () => {
    ...
    test("conductor CAN delegate to plan (not in plan family)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "conductor", abort: new AbortController().signal }
      )
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured advisor to use a specific model in anyon.json
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "advisor", mode: "subagent", model: { providerID: "openai", modelID: "gpt-5.2" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to advisor via subagent_type with user override
      await tool.execute(
        {
          description: "Consult advisor with override",
          prompt: "Review architecture",
          subagent_type: "advisor",
          run_in_background: false,
          load_skills: [],
    ...
    test("agentOverrides variant is applied when model is overridden (#1357)", async () => {
      // given - user configured advisor with model and variant
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         agentOverrides: {
           advisor: { model: "anthropic/claude-opus-4-6", variant: "max" },
         },
       })
      ...
      // when - delegating to advisor via subagent_type with variant override
      await tool.execute(
        {
          description: "Consult advisor with variant",
          prompt: "Review architecture",
          subagent_type: "advisor",
          run_in_background: false,
          load_skills: [],
    ...
    test("fallback chain resolves model when no override and no matchedAgent.model (#1357)", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "advisor", mode: "subagent" }, // no model field
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to advisor with no override and no matchedAgent model
      await tool.execute(
        {
          description: "Consult advisor with fallback",
          prompt: "Review architecture",
          subagent_type: "advisor",
          run_in_background: false,
          load_skills: [],
      ...
      // then - should resolve via AGENT_MODEL_REQUIREMENTS fallback chain for advisor
      // advisor fallback chain: gpt-5.2 (openai) > gemini-3.1-pro (google) > claude-opus-4-6 (anthropic)
  ...
  describe("subagent task permission", () => {
    test("plan subagent should have task permission enabled", async () => {
      //#given - conductor delegates to plan agent
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      //#when - conductor delegates to plan
      await tool.execute(
        {
    ...
    test("non-plan subagent should NOT have task permission", async () => {
      //#given - conductor delegates to advisor (non-plan)
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      // when - conductor delegates to advisor
      await tool.execute(
        {
