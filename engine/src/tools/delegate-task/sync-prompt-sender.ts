...
function isAdvisorAgent(agentToUse: string): boolean {
  return agentToUse.toLowerCase() === "advisor";
}
...
export async function sendSyncPrompt(
  ...
  try {
    ...
    if (
      isAdvisorAgent(input.agentToUse) &&
      isUnexpectedEofError(promptError)
    ) {
      try {
        await deps.promptSyncWithModelSuggestionRetry(client, promptArgs);
        return null;
      } catch (advisorRetryError) {
        promptError = advisorRetryError;
      }
    }
