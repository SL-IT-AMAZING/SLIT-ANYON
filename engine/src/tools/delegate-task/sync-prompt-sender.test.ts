...
bunDescribe("sendSyncPrompt", () => {
  ...
  bunTest("applies agent tool restrictions for scout agent", async () => {
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled();
    bunExpect(promptArgs.body.tools.call_anyon_agent).toBe(false);
  });

  bunTest("applies agent tool restrictions for researcher agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender");
  ...
  bunTest("does not restrict call_anyon_agent for conductor agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender");
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled();
    bunExpect(promptArgs.body.tools.call_anyon_agent).toBe(true);
  });

  bunTest(
    "retries with promptSync for advisor when promptAsync fails with unexpected EOF",
    async () => {
      //#given
      const { sendSyncPrompt } = require("./sync-prompt-sender");
  ...
  bunTest(
    "does not retry with promptSync for non-advisor on unexpected EOF",
    async () => {
      //#given
      const { sendSyncPrompt } = require("./sync-prompt-sender");
