...
describe("resolveSubagentExecution", () => {
  ...
  test("returns delegation error when agent discovery fails instead of silently proceeding", async () => {
    ...
    //#when
    const result = await resolveSubagentExecution(args, executorCtx, "conductor", "deep")

    //#then
    expect(result.agentToUse).toBe("")
    expect(result.categoryModel).toBeUndefined()
    expect(result.error).toBe("Failed to delegate to agent \"advisor\": agents API unavailable")
  })
  ...
  test("logs failure details when subagent resolution throws", async () => {
    ...
    //#when
    await resolveSubagentExecution(args, executorCtx, "conductor", "deep")
