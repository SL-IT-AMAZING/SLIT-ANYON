...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    log(`[anyon] Unsupported platform for ast-grep: ${platformKey}`);
    return null;
  }
  ...
  log(`[anyon] Downloading ast-grep binary...`);
  ...
  try {
    ...
    log(`[anyon] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    log(
      `[anyon] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
