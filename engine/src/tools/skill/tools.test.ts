...
describe("skill tool - agent restriction", () => {
  ...
  it("allows skill when agent matches restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("restricted-skill", { agent: "conductor" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "conductor" }
  ...
  it("throws error when agent does not match restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("conductor-only-skill", { agent: "conductor" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "advisor" }

    // when / #then
    await expect(tool.execute({ name: "conductor-only-skill" }, context)).rejects.toThrow(
      'Skill "conductor-only-skill" is restricted to agent "conductor"'
    )
  })
  ...
  it("throws error when context agent is undefined for restricted skill", async () => {
    ...
    // when / #then
    await expect(tool.execute({ name: "conductor-only-skill" }, contextWithoutAgent)).rejects.toThrow(
      'Skill "conductor-only-skill" is restricted to agent "conductor"'
    )
  })
