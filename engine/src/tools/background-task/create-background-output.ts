...
const WORKER_AGENT = getAgentDisplayName("worker");
...
function formatResolvedTitle(task: BackgroundTask): string {
  const label =
    task.agent === WORKER_AGENT && task.category ? task.category : task.agent;
  ...
  return tool({
    ...
    args: {
      ...
      full_session: tool.schema
        .boolean()
        .optional()
        .describe("Return full session messages with filters (default: true)"),
