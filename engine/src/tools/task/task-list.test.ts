...
describe("createTaskList", () => {
  ...
  beforeEach(() => {
    taskDir = join(testProjectDir, ".anyon/tasks");
  ...
  it("returns empty array when no tasks exist", async () => {
    ...
    const config = {
      conductor: {
        tasks: {
          storage_path: join(testProjectDir, ".anyon/tasks"),
  ...
  it("excludes completed tasks by default", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".anyon/tasks", "T-1.json"), task1);
    writeJsonAtomic(join(testProjectDir, ".anyon/tasks", "T-2.json"), task2);
  ...
  it("returns summary format with id, subject, status, owner, blockedBy", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".anyon/tasks", "T-1.json"), task);
    ...
    expect(summary.owner).toBe("conductor");
  ...
  it("filters blockedBy to only include unresolved (non-completed) blockers", async () => {
    ...
    writeJsonAtomic(
      join(testProjectDir, ".anyon/tasks", "T-blocker-completed.json"),
      blockerCompleted,
    );
    writeJsonAtomic(
      join(testProjectDir, ".anyon/tasks", "T-blocker-pending.json"),
      blockerPending,
    );
    writeJsonAtomic(
      join(testProjectDir, ".anyon/tasks", "T-main.json"),
      mainTask,
    );
