...
console.log("=== Publishing anyon (multi-package) ===\n");
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `@anyon-cli/anyon-${platform}`;
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/SL-IT-AMAZING/anyon-cli/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text();
...
async function gitTagAndRelease(
  ...
  await $`git add package.json assets/anyon.schema.json`;
  for (const platform of PLATFORM_PACKAGES) {
