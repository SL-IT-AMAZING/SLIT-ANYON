# Contributing to Anyon

First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to anyon.
...
git clone https://github.com/SL-IT-AMAZING/anyon-cli.git
cd anyon
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   ```json
   {
     "plugin": ["file:///absolute/path/to/anyon/dist/index.js"]
   }
   ```

   For example, if your project is at `/Users/yourname/projects/anyon`:

   ```json
   {
     "plugin": ["file:///Users/yourname/projects/anyon/dist/index.js"]
   }
   ```

   > **Note**: Remove `"anyon"` from the plugin array if it exists, to avoid conflicts with the npm version.

3. **Restart OpenCode** to load the changes.

4. **Verify** the plugin is loaded by checking for Anyon agent availability or startup messages.
...
anyon/
├── src/
│   ├── index.ts         # Plugin entry (AnyonPlugin)
│   ├── plugin-config.ts # JSONC multi-level config (Zod v4)
│   ├── agents/          # 11 agents (Euler, Tesla, Socrates, Curie, Explore, Turing, Newton, Lovelace, Nietzsche, Multimodal-Looker, Euler-Junior)
...
Thank you for contributing to Anyon! Your efforts help make AI-assisted coding better for everyone.
