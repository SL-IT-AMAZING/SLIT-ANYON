...
**File:** `src/cli/config-manager/write-anyon-config.ts` (line 46)

```typescript
const merged = deepMergeRecord(existing, newConfig);
```

When a user runs `anyon install` (even just to update settings), the installer:
...
- **Affected Files:**
  - `src/cli/config-manager/write-anyon-config.ts`
  - `src/cli/config-manager/deep-merge-record.ts`
- **Trigger:** Running `anyon install` (even for unrelated updates)
...
```bash
cp ~/.config/opencode/anyon.jsonc ~/.config/opencode/anyon.jsonc.backup
```
