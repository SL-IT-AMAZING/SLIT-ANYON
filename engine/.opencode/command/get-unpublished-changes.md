...
<version-context>
<published-version>
!`npm view anyon version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
!`npm view anyon version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null || echo "no commits since release"`
</commits-since-release>
<diff-stat>
!`npm view anyon version 2>/dev/null | xargs -I{} git diff "v{}"..HEAD --stat 2>/dev/null || echo "no diff available"`
</diff-stat>
<files-changed-summary>
!`npm view anyon version 2>/dev/null | xargs -I{} git diff "v{}"..HEAD --stat 2>/dev/null | tail -1 || echo ""`
</files-changed-summary>
</git-context>
...
<socrates-safety-review>
## Socrates Deployment Safety Review (Only when user explicitly requests)

**Trigger keywords**: "safe to deploy", "can I deploy", "is it safe", "review", "check", "socrates"
...
- On failure → Report "❌ Cannot deploy" immediately without invoking Socrates

### 2. Socrates Invocation Prompt

Collect the following information and pass to Socrates:
...
### 3. Output Format After Socrates Response

## 🔍 Socrates Deployment Safety Review Result
