...
- The [euler-bot] prefix is MANDATORY on every comment you post.
...
- The [euler-bot] prefix is MANDATORY on every comment.
...
If the feature already fully exists:
  Post a comment (prefix: [euler-bot]) explaining how to use the existing feature with examples.
  gh issue comment {number} --repo {REPO} --body "YOUR_COMMENT"

RULES:
- Do NOT close feature requests.
- The [euler-bot] prefix is MANDATORY on any comment.
```
...
- The [euler-bot] prefix is MANDATORY on any comment you post.
```
...
| Posting comment without `[euler-bot]` prefix         | CRITICAL |
